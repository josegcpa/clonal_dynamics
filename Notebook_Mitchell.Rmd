...
## Assessing the effect of extending trees

```{r}
par(mfrow = c(4,5),mar = c(2,2,1,1))
for (x in names(bnpr_estimates_trimmed)) {
  for (y in 1:length(bnpr_estimates_trimmed[[x]])) {
    if (length(c(bnpr_estimates_trimmed[[x]][[y]]$coal_times)) > 5) {
      plot_BNPR(bnpr_estimates_extended[[x]][[y]])
      lines(bnpr_estimates_trimmed[[x]][[y]]$summary$time + 15,
            bnpr_estimates_trimmed[[x]][[y]]$summary$quant0.5,
            col = "blue")
      lines(bnpr_estimates[[x]][[y]]$summary$time + 10,
            bnpr_estimates[[x]][[y]]$summary$quant0.5,
            col = "red")
    }
  }
}
...
low_var_estimates %>% 
  ...
  geom_linerange(data = all_vafs_df,
                 aes(x = age,ymin = vaf_005*200e3,ymax = vaf_095*200e3)) +
  geom_point(data = all_vafs_df,aes(x = age,y = vaf * 200e3),size = 0.5) +
  ...
  ylab("Neff") + 
  scale_linetype_manual(values = c(1,3),guide = F) +
  ggsave(filename = sprintf("figures/model_ch/dynamic_coefficients/early_late_difference_trajectory_all.pdf"),
         useDingbats = F,
         width = 5,height = 3)
```