...
n_mut_dev <- 55
ages <- list(`500`=73.4, `2259`=79.4, `3877`=83.8)
av_mut_burden <- list(`500`=1376.4, `2259`=1264.2, `3877`=1577.5)
time_during_development <- list(
  `500` = ages$`500` / av_mut_burden$`500` * n_mut_dev,
  `2259`= ages$`2259` / av_mut_burden$`2259` * n_mut_dev,
  `3877`= ages$`3877` / av_mut_burden$`3877` * n_mut_dev)
...
bnpr_estimates <- trees %>% 
  lapply(
    function(x) {
      ...
      age_w_dev <- ages[[x$ID]] + time_during_development[[x$ID]]
      tree_chron$edge.length <- tree_chron$edge.length * age_w_dev
...
for (item_name in names(bnpr_estimates)) {
  ...
  for (clade_name in names(bnpr_estimates[[item_name]][["clade_dynamics"]])) {
    ...
    sard_id <- item_name
    age_w_dev <- ages[[item_name]] + time_during_development[[item_name]]
    age_range <- full_data %>% 
      subset(SardID == sard_id) %>%
      summarise((min(Age)-age_w_dev),(max(Age)-age_w_dev)) %>%
      unlist %>%
      c
...
for (curr_id in c("2259","500","3877")) {
  ...
  yrs_dev <- time_during_development[[curr_id]]
  age_w_dev <- ages[[curr_id]] + yrs_dev
  max_age <- age_w_dev
  if (curr_id == "2259") {
  ...
  tree_plot <- ggtree(tree_groups,
    ...
    scale_x_continuous(expand = c(0.001,0),
                       breaks = (c(0,20,40,60,80)+yrs_dev) / max_age,
    ...
    if (!grepl("Unknown driver",driver_id)) {
      ...
      trajectory_plot_ <- data.frame(
        ...
        scale_x_continuous(limits = c(0,max_age),
                           breaks = seq(0,100,by = 20) + yrs_dev,
...
for (individual in names(bnpr_estimates)) {
  ...
  yrs_dev <- time_during_development[[individual]]
  for (clade in bnpr_estimate$clade_dynamics) {
    ...
    all_trajectories[[length(all_trajectories) + 1]] <- data.frame(
      X = ages[[individual]] + yrs_dev - clade$bnpr_estimate$summary$time,
...
all_trajectories_df %>%
  ...
  scale_color_manual(values = c(U = "grey40",gene_colours),
                     guide = F) + 
  ggsave("figures/model_ch/trees_development/tree_trajectories.pdf",
         height = 1.5,
         width = 1.8)
...
coefficients_df <- plot_list %>%
  ...
  mutate(
    age_q05 = age_q05 - ifelse(
      source == "Colonies",unlist(time_during_development[as.character(individual)]),0),
    age_q95 = age_q95 - ifelse(
      source == "Colonies",unlist(time_during_development[as.character(individual)]),0)) %>%
...
load_data_6th_tp() %>%
  ...
  theme_gerstung(base_size = 6) +
  ggsave(filename = sprintf("figures/%s/trees_development/compare_vaf.pdf",model_id),
         width = 2,height = 2,
         useDingbats = F)
...
tree_plot_reordered %>% 
  ggsave(filename = sprintf("figures/%s/trees_development/all_trees_and_dynamics_full.pdf",model_id),
         height = 6,
         width = 6,
         useDingbats = F)
tree_plot_reordered %>% 
  ggsave(filename = sprintf("figures/%s/trees_development/all_trees_and_dynamics_full_redux.pdf",model_id),
         height = 5,
         width = 5,
         useDingbats = F)

ggsave(plot = coefficient_age_comparison_plot + theme(strip.text.y = element_text(angle = 90)),
       filename = sprintf("figures/%s/trees_development/coef_age_plot.pdf",model_id),
       width = 2.2,height = 2.5,
       useDingbats = F)

ggsave(plot = linear_coefficients_only_plot + ylab("Growth per year"),
       filename = sprintf("figures/%s/trees_development/linear_coef_only_plot.pdf",model_id),
       width = 2.7,height = 2,
       useDingbats = F)

write.csv(coefficients_df,"data_output/tree_bnpr_coefficients_dev.csv",row.names = T)
