...
mitchell_coefficients <- read.csv("data_output/mitchell_bnpr_coefficients.csv") %>%
  ...
  mutate(source = "Michell et al. 2021") %>%
...
tree_coefficients <- read.csv("data_output/tree_bnpr_coefficients.csv") %>%
  mutate(b_late = b_early + b_late) %>% 
  select(id = individual,clade = plot_label,
         early = b_early,late = b_late,expected = expected_late) %>%
  mutate(plotting_id = "Real data") %>% 
...
all_bnpr_coefficients <- rbind(mitchell_coefficients,tree_coefficients,simulation_coefficients) 
...
rbind(
  all_bnpr_coefficients %>% 
    group_by(source) %>% 
    summarise(N = sum((late / expected) < 0.7,na.rm = T),
              Total = length(id)) %>% 
    mutate(Ratio = N / Total),
  historical_estimates %>% 
    mutate(ratio = (exp(observed_growth_mean)-1) / (exp(historical_growth_mean)-1)) %>% 
    summarise(N = sum(ratio < 0.7,na.rm=T),Total = length(ratio)) %>% 
    mutate(Ratio = N / Total) %>%
    mutate(source = "Fabre et al. 2021 (longitudinal)") %>%
    select(source,N,Total,Ratio)) %>% 
  select(Source = source,`Number of saturating trajectories` = N,`Total number of trajectories` = Total,Proportion = Ratio) %>%
  as.data.frame
