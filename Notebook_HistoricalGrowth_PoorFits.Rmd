...
historical_estimates <- sampling_idxs %>%
  apply(1,function(x) {
    ...
    biologically_feasible <- ages_at_onset > 0 & ages_at_onset < minAge
...
growth_per_year_gene_plot <- historical_estimates %>%
  group_by(gene) %>%
  summarise(average_historical_growth = exp(mean(historical_growth_mean,na.rm=T))-1,
            average_observed_growth = exp(mean(observed_growth_mean,na.rm=T))-1) %>% 
  ggplot(aes(x = reorder(gene,average_observed_growth),
             y = (average_observed_growth + average_historical_growth)/2,
             width = 0.9,
             height = average_historical_growth-average_observed_growth,
             fill = gene)) +  
  ...
  scale_y_continuous(breaks = seq(-0.2,1,by = 0.05),
                     labels = sprintf("%s%%",seq(-0.2,1,by = 0.05)*100),
                     expand = c(0,0,0.01,0)) + 
...
historical_estimates <- historical_estimates %>%
  mutate(
    ratio_q05 = exp(historical_growth_q05-observed_growth_q05),
    ratio_q50 = exp(historical_growth_q50-observed_growth_q50),
    ratio_q95 = exp(historical_growth_q95-observed_growth_q95),
    ratio_mean = exp(historical_growth_mean-observed_growth_mean)
  )
```
...
for (P in unique(tmp$par)) {
  ...
  if (P == "RDW") {
    Mm <- glm(ratio_q50 ~ Gender*meanAge + Mean,data = x,family = Gamma(link = "identity"))
    NMm <- glm(ratio_q50 ~ Gender*meanAge,data = x,family = Gamma(link = "identity"))
    ...
    NMm <- glm(ratio_q50 ~ Gender*meanAge,data = x,family = Gamma(link = "identity"))
    all_comparisons_dnmt3a[[P]] <- data.frame(par = P,anova(NMm,Mm,test = "F")[2,c(5,6)],f = "mean")
    Mm <- glm(ratio_q50 ~ Gender*meanAge + Slope,data = x,family = Gamma(link = "identity"))
    NMm <- glm(ratio_q50 ~ Gender*meanAge,data = x,family = Gamma(link = "identity"))
...
all_comparisons %>%
  ...
  ggsave(useDingbats=FALSE,
         filename = sprintf("figures/%s/phenotype_associations/blood_count_significance.pdf",model_id),
         width = 3,height = 2)
```
