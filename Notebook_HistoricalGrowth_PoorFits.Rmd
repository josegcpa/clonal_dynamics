...
rbind(
  all_bnpr_coefficients %>% 
    group_by(source) %>% 
    summarise(N = sum((late / expected) < 0.44,na.rm = T),
              Total = length(id)) %>% 
    mutate(Ratio = N / Total),
  historical_estimates %>% 
    mutate(ratio = (exp(observed_growth_mean)-1) / (exp(historical_growth_mean)-1)) %>% 
    summarise(N = sum(ratio < 0.44,na.rm=T),Total = length(ratio)) %>% 
    mutate(Ratio = N / Total) %>%
...
```{r historical_recurrence_truncation_dnmt3a_hs,fig.width=2.5,fig.height=1}
tmp_dnmt3a <- tmp %>%
  subset(gene == "DNMT3A") %>%
...
```{r dnds_aml_mds,fig.height=2,fig.width=3.5}
...
```{r aml_risk,fig.height=2,fig.width=3.5}
all_samples <- sampling_idxs %>%
  apply(1,function(x) {
    individual <- x[1]
    site_numeric <- as.numeric(x[3])
...
tmp %>%
  ...
  ggsave(useDingbats=FALSE,sprintf("figures/%s/other_associations/poor_tnt_rec.pdf",model_id),
         height = 1.2,width = 2.0) 
anvar_test
```
...
significance_df <- all_models %>% 
  lapply(
    function(x) anova(x$null_m,x$m,test="Chisq")[2,c(4,5)]
  ) %>%
  do.call(what = rbind)
significance_df$gene <- names(all_models)
significance_df$SignificantAfterCorrection <- p.adjust(significance_df$`Pr(>Chi)`,method = "BH") < 0.05
...
tmp %>% 
  ...
  ggsave(useDingbats=FALSE,sprintf("figures/%s/other_associations/poor_smoke_sex.pdf",model_id),
         height = 1.2,width = 2.0) 
```
