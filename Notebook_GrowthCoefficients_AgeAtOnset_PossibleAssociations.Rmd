...
tmp_data_per_individual <- load_data_keep() %>%
  subset(VAF >= 0.005) %>% 
  group_by(SardID) %>% 
  mutate(Age = max(Age)) %>% 
  group_by(SardID,Age,Gender) %>%
  summarise(TET2 = sum(grepl("TET2",unique(AAChange.refGene))),
            DNMT3A = sum(grepl("DNMT3A",unique(AAChange.refGene))),
            Splicing = sum(grepl("U2AF1|SRSF2|SF3B1",unique(AAChange.refGene)))) %>%
  mutate(AgeGroup = cut(Age,c(0,75,80,85,200),c("<75","75-80","80-85",">85"))) %>%
  mutate(AgeGroup = factor(AgeGroup,levels = c("<75","75-80","80-85",">85"))) %>%
  group_by(AgeGroup) %>% 
  mutate(AgeGroup_ = sprintf("%s\n(N=%s)",AgeGroup,length(unique(SardID)))) %>%
  gather(key = "key",value = "value",-Age,-AgeGroup,-AgeGroup_,-SardID,-Gender) %>%
  mutate(key = factor(key, levels = c("DNMT3A","TET2","Splicing"),
...
tmp_data_agg %>% 
  ...
  geom_linerange(aes(ymin = p05,ymax = p95),position = position_dodge(width = 0.9),
                 size = 0.25) +
  geom_point(data = tmp_data_per_individual,aes(y = value/10),size = 0.1,
             alpha = 0.5,
  ...
  scale_y_continuous(expand = c(0,0),limits = c(0,mm),
                     sec.axis = sec_axis(trans = ~ .*10,name = "Mutations per individual (.)")) + 
...
rbind(tmp_data_agg,tmp_data_splicing_agg) %>%
  select(-AgeGroup_,TotalIndividuals = Total) %>%
  write.csv("data_output/figure_1d.csv")
```

Same as before but considering only *TET2* and *DNMT3A*.
...
load_data_keep() %>%
    ...
    mutate(AgeGroup = cut(Age,c(0,75,80,85,200),c("<75","75-80","80-85",">85"))) %>% 
    group_by(SardID,AgeGroup,Age,Gender) %>%
    summarise(TET2 = sum(ifelse(VAF >= 0.005,grepl("TET2",unique(AAChange.refGene)),F)),
              DNMT3A = sum(ifelse(VAF >= 0.005,grepl("DNMT3A",unique(AAChange.refGene)),F)),
              Splicing = sum(ifelse(VAF >= 0.005,grepl("SF3B1|U2AF1|SRSF2",unique(AAChange.refGene)),F)),
              TotalClones = length(unique(AAChange.refGene)),
              Total = 1) %>%
    group_by(AgeGroup) %>% 
    summarise(TotalTET2 = sum(TET2,na.rm=T),
              TotalDNMT3A = sum(DNMT3A,na.rm=T),
              TotalSplicing = sum(Splicing,na.rm=T),
              TotalClones = sum(TotalClones),
              Total = length(unique(paste(SardID,Age)))) %>%
    gather(key = "key",value = "value",-AgeGroup,-Total,-TotalClones) %>%
    rowwise() %>%
    ...
    mutate(AgeGroup_ = sprintf("%s\n(N=%s)",AgeGroup,Total)) %>% 
    subset(key != 'Splicing') %>%
    group_by(AgeGroup) %>% mutate(TT = sum(value)) %>%
    ggplot(aes(x = "", y = value/TT,fill = key)) + 
  geom_bar(stat="identity") + 
  facet_wrap(~ AgeGroup) + 
  ...
  theme(axis.text = element_blank(),axis.ticks = element_blank(),
        ...
        legend.key.size = unit(0.2,"cm"),legend.text = element_text(face = "italic")) +
  ggtitle("All clones")
...
load_data_keep() %>%
    ...
    mutate(AgeGroup = cut(Age,c(0,75,80,85,200),c("<75","75-80","80-85",">85"))) %>% 
    group_by(SardID,AgeGroup,Age,Gender) %>%
    summarise(TET2 = sum(ifelse(VAF >= 0.025,grepl("TET2",unique(AAChange.refGene)),F)),
              DNMT3A = sum(ifelse(VAF >= 0.025,grepl("DNMT3A",unique(AAChange.refGene)),F)),
              Splicing = sum(ifelse(VAF >= 0.025,grepl("SF3B1|U2AF1|SRSF2",unique(AAChange.refGene)),F)),
              TotalClones = length(unique(AAChange.refGene)),
              Total = 1) %>%
    group_by(AgeGroup) %>% 
    summarise(TotalTET2 = sum(TET2,na.rm=T),
...
statistic_full <- statistic_full_tails %>%
  ...
  summarise(sum_stat = all(tail_prob > 0.025 & tail_prob < 0.975),
            ...
            n_tp = length(unique(age))) %>%
  mutate(split = 'full') %>%
  select(split,site,sum_stat,sum_stat_005,sum_stat_095,gene = genes,individual,sum_stat_genetic,n_tp) %>%
  ungroup() 
...
```{r model_ch_dynamic_visualization_all_full,fig.width = 8.3,fig.height = 11.7}
...
r_values_merged_prediction_only <- r_values_merged %>%
  ...
  mutate(M = diff(log(c(value[which.min(age)]+1e-5,value[which.max(age)])))) %>%
  group_by(individual) %>%
  mutate(M = max(M,na.rm=T))

M_order <- r_values_merged_prediction_only %>%
  ungroup() %>%
  select(individual,M) %>%
  distinct %>%
  arrange(M) %>%
  ...
  select(individual) %>%
  unlist
M_order <- M_order[2:length(M_order)] # keeps it nice and tidy
colour_breaks <- r_values %>% 
  select(gene,site,individual,b_site,b_gene) %>% 
  distinct %>% mutate(gene = str_match(gene,'[A-Z0-9]+')) %>% 
  group_by(gene) %>% summarise(M = mean(b_site+b_gene)) %>% 
  arrange(M) %>%
  ungroup %>%
  select(gene) %>% 
  unlist
...
ggplot(data = NULL) +
  geom_ribbon(data = r_values_merged_prediction_only,
              aes(ymin = pred_005,
                  ...
                  alpha = 0.1),
              na.rm = T) +
  geom_line(data = r_values_merged_data_only,
            aes(x = age,y = `Observed VAF`,
                colour = str_match(gene,'[A-Z0-9]+'),group = group),
  ...
  geom_line(data = r_values_merged_prediction_only,
            aes(x = age,y = `Inferred VAF`,
                colour = str_match(gene,'[A-Z0-9]+'),group = group),
            size = 0.5) +
  facet_wrap(~ factor(individual,levels = M_order),scales="free",ncol = 11) + 
  scale_color_manual(name=NULL,values = gene_colours,breaks = colour_breaks) +
  theme_gerstung(base_size = 6) + 
  theme(strip.text = element_blank(),axis.text = element_blank(),
        legend.key.height = unit(0,"cm")) +
  xlab("Age") + 
  ylab("VAF") + 
  scale_alpha(range = c(0.05,0.05),guide=F) +
  scale_y_continuous(n.breaks = 4,trans = 'log10',
...
truncating_recurring_uc <- r_values %>%
  ...
  mutate(f = paste(
    ...
    ifelse(truncating,"truncating","missense"),
    sep = ' and\n'
  ))
...
```{r unknown_cause_sex_smoked_association, fig.width=2,fig.height=1.3}
...
all_gene_except_tet2_pearson_correlation <- cor.test(
  subset(Parameters,gene != 'TET2')$age,subset(Parameters,gene != 'TET2')$b_clone_mean)
pearson_correlations <- rbind(
  ...
  data.frame(
    gene = "All genes except TET2",
    rho = all_gene_except_tet2_pearson_correlation$estimate,
    p.val = all_gene_except_tet2_pearson_correlation$p.value,
    stringsAsFactors = F
  )
)
...
min_max_plots <- c(min(c(r_values$coefficient_005,r_values$b_clone)),
                   max(c(r_values$coefficient_095,r_values$b_clone)))
min_max_plots <- exp(min_max_plots) * 100 - 100
min_max_plots_round <- round(min_max_plots,-1)

tmp_2 <- r_values %>%
  mutate(vaf = true/coverage) %>%
  group_by(individual,site,b_clone,b_genetic_mean,genes) %>%
  summarise(vaf_strict = vaf[vaf > 0.005][which.min(age[vaf > 0.005])],
            vaf_median = median(vaf[vaf > 0.005],na.rm=T),
            vaf_last = vaf[which.max(age)],
            vaf = vaf[which.min(age)],
            age = mean(age)) %>% 
  ungroup %>% 
  mutate(genes = str_match(genes,'[A-Z0-9]+')) %>% 
...
uc_vaf_plot <- tmp_2 %>%
  ggplot(aes(x = vaf,y = exp(b_clone)*100-100,colour = genes)) + 
  geom_point(alpha = 0.4,size = 0.5) + 
  theme_gerstung(base_size = 6) + 
  xlab("VAF at study entry") + 
  ylab("UC effect") + 
  scale_color_manual(values = gene_colours,name = "Gene") + 
  guides(colour=guide_legend(ncol=2,byrow=F)) +
  theme(legend.position = 'none') +
...
uc_vaf_last_plot <- tmp_2 %>%
  ggplot(aes(x = vaf_last,y = exp(b_clone)*100-100,colour = genes)) + 
  geom_point(alpha = 0.4,size = 0.5) + 
  theme_gerstung(base_size = 6) + 
  xlab("VAF at study exit") + 
  ylab("UC effect") + 
  scale_color_manual(values = gene_colours,name = "Gene") + 
  guides(colour=guide_legend(ncol=2,byrow=F)) +
  theme(legend.position = 'none') +
...
```{r unknown_cause_age_association,fig.width=2,fig.height=1.3}
ggsave(plot = uc_age_plot,useDingbats=FALSE,sprintf("figures/%s/dynamic_coefficients/uc_age.pdf",model_id),
       height = 1.3,width = 2) 

ggsave(plot = uc_vaf_plot,useDingbats=FALSE,sprintf("figures/%s/dynamic_coefficients/uc_vs_vaf.pdf",model_id),
       height = 1.3,width = 2) 

ggsave(plot = uc_vaf_last_plot,sprintf("figures/%s/dynamic_coefficients/uc_vs_vaf_last.png",model_id,dpi = 300),
       height = 1.3,width = 2) 
...
tmp_ <-  tmp_2 %>% 
  subset(vaf > 0 & vaf_last > 0) %>%
  mutate(log_vaf = log(vaf),
         log_vaf_last = log(vaf_last),
         log_vaf_median = log(vaf_median),
         log_vaf_strict = log(vaf_strict),
         D = vaf_last / vaf)

cor.test(tmp_$log_vaf,tmp_$b_clone) 

cor.test(tmp_$log_vaf_last,tmp_$b_clone)

glm(b_clone ~ log_vaf + log_vaf : genes,data = tmp_) %>%
  summary

tmp_ %>% 
  ggplot(aes(x = vaf, y = b_clone,colour = genes)) + 
...
age_threshold_accounting <- Parameters_Age %>%
  ...
  transmute(
    limit = onset_from_detection(
      b_clone_mean+b_genetic_mean,minVAF,minAge,2,N),
    age_at_onset = Q50,
...
dff %>%
  ...
  mutate(gene = ifelse(grepl("DNMT",site),str_match(as.character(site),"[A-Z0-9]+[tn]+"),
                       str_match(as.character(site),"[A-Z0-9]+"))) %>% 
...
smoking_clone_ages <- load_smoking_data() %>% 
  ...
  mutate(effect = Q50) %>%
  filter(age == max(age)) %>%
  select(SardID,effect,site,CurrentSmoker, QuitYears,status,age,genes,Gender,b_clone_mean) %>% 
...
```{r predictive_power_6th_tp,fig.width=4,fig.height=2,warning=F,message=F}
beta_value <- values_model$beta_values[[1]][1,1]
...
pred_true_change <- last_tp %>%
  ...
  summarise(pred_increase = pred[which.max(Age)]/pred[which.max(Age)-1],
            ...
            pred_005 = pred_005[which.max(Age)],
            pred = pred[which.max(Age)],
            pred_095 = pred_095[which.max(Age)],
            vaf = VAF[which.max(Age)]) 
...
performance_scatter_plot <- pred_true_change %>%
  ...
  geom_point(size=0.8) + 
  xlab("Observed fold change in VAF") + 
  ylab("Predicted fold change in VAF")  + 
  theme_gerstung(base_size = 6) +
...
plot_min_max_ <- c(
  0.005,
  max(pred_true_change$pred_095,pred_true_change$vaf)
)

pred_true_change %>% 
  ggplot(aes(x = vaf,y = pred,ymin = pred_005,ymax = pred_095,colour = gene)) + 
  geom_abline(intercept = 0,slope = 1,linetype = 3) + 
  geom_point(size = 0.5) + 
  geom_linerange(size = 0.25) + 
  coord_cartesian(xlim = plot_min_max_,ylim = plot_min_max_) +
  xlab("Observed VAF") + 
  ylab("Predicted VAF")  + 
  theme_gerstung(base_size = 6) +
  scale_x_continuous(trans = 'log10') + 
  ...
  ggsave(useDingbats=FALSE,
         sprintf("figures/%s/inferred_trajectories/performance_6tp.pdf",model_id),
         height = 1.5,width = 2.3)
...
statistics_data <- statistics_lists$individual_site %>% 
  select(site,sum_stat,gene,individual,sum_stat_genetic,n_tp) %>% 
  mutate(model = "") %>%
...
stats_nmut %>%
  ...
  summarise(N=sum(sum_stat),Total=length(sum_stat),
            PropGood = sum(sum_stat)/length(sum_stat),
...
```{r explained_n_tp,fig.height=1.5,fig.width=1.4}
statistics_data %>% 
  group_by(n_tp) %>% 
  summarise(N = sum(sum_stat),Total = length(sum_stat),
            q05 = qbeta(0.05,N,Total-N),
            q95 = qbeta(0.95,N,Total-N)) %>% 
  ggplot(aes(x = n_tp,y = N/Total)) + 
  geom_bar(stat = "identity",fill = "grey80") + 
  geom_linerange(aes(ymin = q05,ymax = q95)) + 
  theme_gerstung(base_size = 6) + 
  ylab("Fraction of explained trajectories") + 
  xlab("Number of available\ntimepoints") + 
  scale_y_continuous(expand = c(0,0)) + 
  ggsave(useDingbats=FALSE,sprintf("figures/%s/statistical_analysis/statistics_vs_n_tp.pdf",model_id),
...
```{r,fig.height=1.9,fig.width=4}
statistics_data %>% 
  subset(gene %in% c("DNMT3A","TET2","ASXL1","PPM1D","TP53","SF3B1")) %>% 
  group_by(n_tp,gene) %>% 
  summarise(N = sum(sum_stat),Total = length(sum_stat),
            q05 = qbeta(0.05,N,Total-N),
            q95 = qbeta(0.95,N,Total-N)) %>% 
  mutate(gene = factor(gene,levels = c("TET2","DNMT3A","SF3B1","ASXL1","TP53","PPM1D"))) %>% 
  subset(n_tp > 2) %>%
  ggplot(aes(x = factor(n_tp,levels=c(5,4,3,2)))) + 
  geom_bar(aes(y = Total/Total),stat = "identity",fill = "red4",
           colour = "black",size = 0.25) +
  geom_bar(aes(y = N/Total),stat = "identity",fill = "green3",
           colour = "black",size = 0.25) + 
  geom_text(aes(y = 1,label = sprintf("%.2f%% (%s/%s)",N/Total*100,N,Total)),
            size=2,hjust=-0.02) +
  theme_gerstung(base_size = 6) + 
  ylab("Fraction of explained trajectories") + 
  xlab("Number of available timepoints") + 
  scale_y_continuous(expand = c(0,0),breaks = c(0,25,50,75,100)/100) + 
  facet_wrap(~ gene,ncol=2) +
  coord_flip(ylim = c(NA,1.6)) +
  ggsave(useDingbats=FALSE,sprintf("figures/%s/statistical_analysis/statistics_vs_n_tp_by_gene.pdf",model_id),
         height = 1.9,width = 4) 
```
...
p_u2af1_c_mds <- 21.7/100 # Wang 2020 (Sci. rep.)
p_u2af1_c_cml <- 13.1/100 # Abu Kar 2013 (Haematologica)
p_u2af1_c_aml <- 9/100 # Bamopoulos 2020 (Leukaemia)
...
S_u2af1 <- load_data() %>% 
  ...
  summarise(`U2AF1 in our cohort` = length(unique(SardID[Gene == 'U2AF1'])) / length(unique(SardID)),
            ...
            `U2AF1 mutation in CMML` = p_u2af1_c_cml,
  ...
  mutate(Condition = factor(
    ...
    levels = c("U2AF1 in our cohort at constant mutation rate",
               ...
               "U2AF1 mutation in CMML","U2AF1 mutation in AML")))
...
S <- load_data() %>% 
  ...
  summarise(`SRSF2-P95H in our cohort` = length(unique(SardID[amino_acid_change == 'SRSF2-P95H'])) / length(unique(SardID)),
            ...
            `SRSF2 mutation of myeloid origin` = o,
  ...
  mutate(Condition = factor(
    ...
    levels = c("SRSF2-P95H in our cohort at constant mutation rate",
               "SRSF2-P95H in our cohort","SRSF2 mutation of myeloid origin",
...
all_gene_vaf <- full_data %>%
  group_by(SardID,amino_acid_change) %>%
  mutate(medianVAF = median(VAF[VAF >= 0.005]),
         randomVAF = sample(VAF[VAF >= 0.005],1)) %>%
  group_by(SardID) %>% 
  filter(Age %in% range(Age)) %>%
  mutate(Age = ifelse(Age == min(Age),"Entry","Exit")) %>%
  select(-relative_timepoint,-Phase,-MUTcount_Xadj,-WTcount,-TOTALcount) %>%
  spread(key = Age,value = VAF) %>%
  mutate(Gene = str_match(Gene,'[A-Z0-9]+')) %>%
  group_by(Gene) %>%
  summarise(meanlogVAF = log(median(Entry,na.rm=T)),
            medianMedianVAF = log(median(medianVAF)),
            medianRandomVAF = log(median(randomVAF)),
            meanlogVAFStrict = log(median(Entry[Entry == 0 | Entry >= 0.005],na.rm=T)),
            meanlogVAFLate = log(median(Exit,na.rm=T)),
            IncipientRatio = sum(Entry == 0,na.rm=T)/length(Entry)) %>%
  select(gene = Gene,meanlogVAF,meanlogVAFLate,IncipientRatio,medianMedianVAF,medianRandomVAF)

dd <- all_samples %>% 
  merge(read.csv("data/aml-risk-coef.csv"),by = "gene",all = F) %>% 
  merge(all_gene_vaf,by = "gene",all = F) %>%
  as.data.frame
...
### Can VAF during study be a possible confounder?

In Abelson & Collord *et al.* (2018), clone size has a sizeable impact on AML risk, so it is important to assess this.

```{r,fig.height=2.5,fig.width=6}
n <- nrow(dd)
correlations <- cor(dd[,c(2,5,11,12)]) %>%
  as.data.frame
colnames(correlations) <- c("Average growth\nperyear","AML risk",
                            "VAF at\nstudy entry","VAF at\nstudy exit")
correlations$ids <- c("Average growth\nperyear","AML risk",
                      "VAF at\nstudy entry","VAF at\nstudy exit")
correlations_long <- correlations %>%
  gather(key = "key",value = "R",-ids)
correlations_long <- correlations_long %>%
  mutate(t.val = abs(R)*sqrt(n-2)/(sqrt(1-(R^2)))) %>%
  mutate(p.val = (1-pt(t.val,df = n-2))*2) %>% 
  mutate(sig = cut(p.val,c(0,0.001,0.005,0.05,1),c("***","**","*","")))
  
plot_grid(
  plot_grid(
    dd %>%
      ggplot(aes(x = exp(meanlogVAF),y = exp(meanlogVAFLate),colour = gene)) +
      geom_smooth(size = 0.25,alpha = 0.5,method = "lm",formula = y ~ x,colour = "black",
                  linetype = 3) +
      geom_point(size = 0.5) + 
      theme_gerstung(base_size = 6) +
      theme(legend.position = "none") + 
      xlab("Median VAF at study entry") + 
      ylab("Median VAF at study exit") + 
      scale_y_continuous(trans = 'log10') + 
      scale_x_continuous(trans = 'log10') +
      scale_colour_manual(values = gene_colours),
    dd %>%
      ggplot(aes(x = exp(meanlogVAF),y = mean,colour = gene)) +
      geom_smooth(size = 0.5,alpha = 0.5,method = "lm",formula = y ~ x,colour = "black") +
      geom_point(size = 0.5) + 
      theme_gerstung(base_size = 6) +
      theme(legend.position = "none") + 
      xlab("Median VAF at study entry") + 
      ylab("Average growth per year") + 
      scale_x_continuous(trans = 'log10') +
      scale_colour_manual(values = gene_colours),
    dd %>%
      ggplot(aes(x = exp(meanlogVAFLate),mean,colour = gene)) +
      geom_smooth(size = 0.5,alpha = 0.5,method = "lm",formula = y ~ x,colour = "black") +
      geom_point(size = 0.5) + 
      theme_gerstung(base_size = 6) +
      theme(legend.position = "none") + 
      xlab("Median VAF at study exit") + 
      ylab("Average growth per year") + 
      scale_x_continuous(trans = 'log10') +
      scale_colour_manual(values = gene_colours),
    dd %>%
      ggplot(aes(x = exp(meanlogVAF),coef,colour = gene)) +
      geom_smooth(size = 0.5,alpha = 0.5,method = "lm",formula = y ~ x,colour = "black") +
      geom_point(size = 0.5) + 
      theme_gerstung(base_size = 6) +
      theme(legend.position = "none") + 
      ...
      scale_colour_manual(values = gene_colours),
    dd %>%
      ggplot(aes(x = exp(meanlogVAFLate),coef,colour = gene)) +
      geom_smooth(size = 0.25,alpha = 0.5,method = "lm",formula = y ~ x,colour = "black",
                  linetype = 3) +
      geom_point(size = 0.5) + 
      theme_gerstung(base_size = 6) +
      ...
      scale_colour_manual(values = gene_colours),
    dd %>%
      ggplot(aes(x = mean,coef,colour = gene)) +
      geom_smooth(size = 0.5,alpha = 0.5,method = "lm",formula = y ~ x,colour = "black") +
      geom_point(size = 0.5) + 
      theme_gerstung(base_size = 6) +
      theme(legend.position = "none") + 
      xlab("Average growth per year") + 
      ylab("AML risk") + 
      scale_colour_manual(values = gene_colours)),
  plot_grid(
    NULL,
    correlations_long %>%
      ggplot(aes(x = ids,y = key,fill = R)) + 
    ...
    NULL,ncol = 1,rel_heights = c(0.3,1,0.3)),
  nrow = 1,rel_widths = c(2,1)) + 
  ggsave(useDingbats=FALSE,
         filename = sprintf("figures/%s/dynamic_coefficients/aml_risk_full.pdf",model_id),
         width = 6,height = 2.5)
```

```{r,fig.height=1.3,fig.width=6}
n <- nrow(dd)
correlations <- cor(dd[,c(2,5,11)]) %>%
  as.data.frame
colnames(correlations) <- c("Average growth\nperyear","AML risk",
                            "VAF at\nstudy entry")
correlations$ids <- c("Average growth\nperyear","AML risk",
...
plot_grid(
  ...
  plot_grid(
    correlations_long %>%
      ...
      scale_y_discrete(expand = c(0,0)) + 
      ggtitle("D") +
      rotate_x_text(angle = 90),ncol = 1),
  nrow = 1,rel_widths = c(2,1)) + 
  ggsave(filename = sprintf("figures/%s/dynamic_coefficients/aml_risk_early_vaf.png",model_id),
         width = 6,height = 1.3)
```

```{r,fig.height=2.6,fig.width=6}
n <- nrow(dd)
correlations <- cor(dd[,c(2,5,11,14)]) %>%
  as.data.frame
colnames(correlations) <- c("Average growth\nperyear","AML risk",
                            "VAF at\nstudy entry","Median\naverage VAF")
correlations$ids <- c("Average growth\nperyear","AML risk",
...
plot_grid(
  plot_grid(
    dd %>%
      ggplot(aes(x = exp(meanlogVAF),y = exp(medianMedianVAF),colour = gene)) +
      geom_smooth(size = 0.5,alpha = 0.5,method = "lm",formula = y ~ x,colour = "black") +
      geom_point(size = 0.5) + 
      theme_gerstung(base_size = 6) +
      theme(legend.position = "none") + 
      xlab("Median VAF at study entry") + 
      ylab("Median average VAF") + 
      scale_x_continuous(trans = 'log10') +
      ggtitle("A") +
      scale_colour_manual(values = gene_colours),
    dd %>%
    ...
    dd %>%
      ...
      scale_colour_manual(values = gene_colours),
    dd %>%
      ggplot(aes(x = exp(medianMedianVAF),y = mean,colour = gene)) +
      geom_smooth(size = 0.5,alpha = 0.5,method = "lm",formula = y ~ x,colour = "black") +
      geom_point(size = 0.5) + 
      theme_gerstung(base_size = 6) +
      theme(legend.position = "none") + 
      xlab("Median average VAF") + 
      ylab("Average growth per year") + 
      scale_x_continuous(trans = 'log10') +
      ggtitle("D") +
      scale_colour_manual(values = gene_colours),
    dd %>%
      ggplot(aes(x = exp(medianMedianVAF),y = coef,colour = gene)) +
      geom_smooth(size = 0.5,alpha = 0.5,method = "lm",formula = y ~ x,colour = "black") +
      geom_point(size = 0.5) + 
      theme_gerstung(base_size = 6) +
      theme(legend.position = "none") + 
  ...
  plot_grid(
    ...
    correlations_long %>%
      ...
      scale_y_discrete(expand = c(0,0)) + 
      ggtitle("F") +
      rotate_x_text(angle = 90),ncol = 1,rel_heights = c(0.5,1)),
  nrow = 1,rel_widths = c(2,1)) + 
  ggsave(filename = sprintf("figures/%s/dynamic_coefficients/aml_risk_early_vaf_median_vaf.png",model_id),
         width = 6,height = 2.6)
```

```{r,fig.height=2.6,fig.width=6}
n <- nrow(dd)
correlations <- cor(dd[,c(2,5,11,15)]) %>%
  as.data.frame
colnames(correlations) <- c("Average growth\nperyear","AML risk",
                            "VAF at\nstudy entry","Median\nrandom VAF")
correlations$ids <- c("Average growth\nperyear","AML risk",
...
plot_grid(
  plot_grid(
    dd %>%
      ggplot(aes(x = exp(meanlogVAF),y = exp(medianRandomVAF),colour = gene)) +
      geom_smooth(size = 0.5,alpha = 0.5,method = "lm",formula = y ~ x,colour = "black") +
      geom_point(size = 0.5) + 
      theme_gerstung(base_size = 6) +
      theme(legend.position = "none") + 
      xlab("Median VAF at study entry") + 
      ylab("Median random VAF") + 
      scale_x_continuous(trans = 'log10') +
      ggtitle("A") +
      scale_colour_manual(values = gene_colours),
    dd %>%
    ...
    dd %>%
      ...
      scale_colour_manual(values = gene_colours),
    dd %>%
      ggplot(aes(x = exp(medianRandomVAF),y = mean,colour = gene)) +
      geom_smooth(size = 0.5,alpha = 0.5,method = "lm",formula = y ~ x,colour = "black") +
      geom_point(size = 0.5) + 
      theme_gerstung(base_size = 6) +
      theme(legend.position = "none") + 
      xlab("Median random VAF") + 
      ylab("Average growth per year") + 
      scale_x_continuous(trans = 'log10') +
      ggtitle("D") +
      scale_colour_manual(values = gene_colours),
    dd %>%
      ggplot(aes(x = exp(medianRandomVAF),y = coef,colour = gene)) +
      geom_smooth(size = 0.5,alpha = 0.5,method = "lm",formula = y ~ x,colour = "black") +
      geom_point(size = 0.5) + 
      theme_gerstung(base_size = 6) +
      theme(legend.position = "none") + 
  ...
  plot_grid(
    ...
    correlations_long %>%
      ...
      rotate_x_text(angle = 90),ncol = 1,rel_heights = c(0.5,1)),
  nrow = 1,rel_widths = c(2,1)) + 
  ggsave(filename = sprintf("figures/%s/dynamic_coefficients/aml_risk_early_vaf_random_vaf.png",model_id),
         width = 6,height = 2.6)
```
