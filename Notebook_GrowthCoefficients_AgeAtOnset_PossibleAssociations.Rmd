...
```{r model_f2_dynamic_visualization_all,fig.width = 6,fig.height = 5}
...
```{r model_f2_gene_coefficients, fig.width=5.6, fig.height=2.4}
...
```{r model_f2_site_coefficients, fig.width=5.6, fig.height=2.4}
...
```{r model_f2_full_effects, fig.width=5.6, fig.height=2.4,warning=F}
...
```{r model_f2_full_effects_simplified, fig.width=5.4, fig.height=2.2,warning=F}
...
We conduct this analysis to better understand the impacts that different factos may have on the unknown cause effect. We begin by estimating the within and between individual variance. We show here that there is no difference between inter- and intra-individual variance, which would further confirm the independence of clone onset even within individuals. 
...
```{r mutation_independent_per_gene,fig.width=1.9,fig.height=1.6,dpi=300}
...
for (gen_time in c(1,2,5,10)) {
  for (pop_size in c(5e4,1e5,2e5)) {
...
stratified_srsf2_samples <- all_samples_for_age_estimation %>%
  ...
  mutate(age_distribution = ifelse(age_distribution < 0,0,age_distribution)) %>% 
  group_by(strata) %>% 
  summarise(N = length(unique(paste(individual,site))),
            Mean = mean(age_distribution,na.rm = T),
            Q05 = quantile(age_distribution,0.05,na.rm = T),
            Q50 = quantile(age_distribution,0.50,na.rm = T),
            Q95 = quantile(age_distribution,0.95,na.rm = T)) %>%
  select(gene = strata,N,Mean,Q05,Q50,Q95)
...
max_growth_fitness_genes <- Parameters_Age %>%
  ...
  merge(estimates_from_samples,by = "gene") %>%
...
estimates_from_samples %>% 
  ggplot(aes(ymin = Q05,ymax = Q95,
             ...
             fill = gene)) +
  geom_tile(aes(y = (90 + Q05)/2,width = 0.8,height = (90 - Q05)),
            fill = "grey90") + 
  geom_tile(aes(y = (Q95 + Q05)/2,width = 0.8,height = Q95 - Q05)) + 
...
Here we inspect how a different number of generations per year affects our interval estimates.

```{r compare_genes_plausibility,fig.height=2,fig.width=3}
Parameters_Age %>%
  group_by(gene) %>% 
  summarise(N = sum(Q50 < 0),
            Total = length(Q50)) %>%
  mutate(Q05 = qbeta(0.05,N,Total-N),
         Q95 = qbeta(0.95,N,Total-N)) %>%
  ggplot(aes(x = reorder(gene,N/Total),y = N / Total,
             ymin = Q05,ymax = Q95)) +
  geom_bar(stat = "identity") +
  geom_linerange() +
  coord_flip() + 
  ...
  theme_gerstung(base_size = 6) + 
  xlab("Gene") + 
  ylab("Proportion of biologically unfeasible clones") + 
  theme(axis.text.y = element_text(face = "italic"))
```

```{r compare_gen_time_pop_size,fig.width=4.5,fig.height=6}
...
all_samples_for_age_estimation_gene_compare %>%
  do.call(what = rbind) %>%
  rowwise() %>%
  mutate(
  ...
  xlab("Population size") + 
  ylab("Age estimates") + 
  scale_color_manual(values = pal_gsea(n = 4)(4),
                     name = "Generations/year") +
  theme(legend.position = "bottom",
        legend.key.size = unit(0.1,"cm")) + 
  ggsave(useDingbats=FALSE,
         sprintf("figures/%s/age_at_clone_foundation/estimate_variability.pdf",model_id),width = 4.5,height = 6)
```
...
statistics_data_bars %>% 
  ...
  ggsave(useDingbats=FALSE,sprintf("figures/%s/statistical_analysis/statsitcs_barplot.pdf",model_id),
         height = 3.3,width = 2) 
...
slope_intercept_blood <- slope_intercept_blood_full %>%
  group_by(SardID) %>%
  mutate(maxVAF = max(maxVAF)) %>% 
  subset(gene == "DNMT3A") %>% 
  mutate(maxVAF = ifelse(is.na(maxVAF),0,maxVAF),
         b_clone_mean = ifelse(is.na(b_clone_mean),0,b_clone_mean),
         total_effect = ifelse(is.na(total_effect),0,total_effect)) %>% 
  group_by(SardID,gene,par,Mean,Slope,meanAge,Gender,maxVAF) %>%
  summarise(b_clone_mean = b_clone_mean[which.max(maxVAF)],
            site = site[which.max(maxVAF)]) %>%
  ungroup 
...
for (x in unique(slope_intercept_blood$par)) {
  ...
  if (x == "RDW") {
    sub_data <- slope_intercept_blood %>%
      ...
      subset(!is.na(Mean)) %>%
      mutate(gene = str_match(site,'[A-Z0-9]+')) %>%
      select(Mean,meanAge,Gender,maxVAF,b_clone_mean,site)
    tmp$mean$model <- glm(Mean ~ maxVAF + Gender*meanAge + b_clone_mean,data=sub_data)
    tmp$mean$model_null <- glm(Mean ~ maxVAF + Gender*meanAge,data=sub_data)
    tmp$slope$model <- NULL
    tmp$slope$model_null <- NULL
    tmp$mean$data <- cbind(sub_data,
                           Pred = predict(tmp$mean$model,newdata=sub_data))
    tmp$slope$data <- NULL
    tmp$mean$coefficients <- cbind(as.data.frame(coef(summary(tmp$mean$model))),
                                   StandardizedEstimate = beta(tmp$mean$model,y=T)$coef[,1],
                                   StandardizedError = beta(tmp$mean$model,y=T)$coef[,2],
                                   coef = names(tmp$mean$model$coefficients))
    tmp$slope$coefficients <- NULL
    tmp$mean$rank_correlation <- cor.test(sub_data$Mean,sub_data$b_clone_mean,method = "spearman")
  } else {
    sub_data <- slope_intercept_blood %>%
      ...
      subset(!is.na(Mean) & !is.na(Slope) & !is.na(maxVAF) & !is.na(b_clone_mean)) %>%
      mutate(gene = str_match(site,'[A-Z0-9]+')) %>%
      select(Mean,Slope,meanAge,Gender,maxVAF,b_clone_mean,site)
    tmp$mean$model <- glm(Mean ~ maxVAF + Gender*meanAge + b_clone_mean,data=sub_data)
    tmp$mean$model_null <- glm(Mean ~ maxVAF + Gender*meanAge,data=sub_data)
    tmp$slope$model <- glm(Slope ~ maxVAF + Gender*meanAge + b_clone_mean,data=sub_data)
    tmp$slope$model_null <- glm(Slope ~ maxVAF + Gender*meanAge,data=sub_data)
