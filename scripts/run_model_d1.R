...
b_values <- calculate(full_effects,draws) %>% lapply(function(x) tail(x,5000) %>% variable_summaries)
interference_values <- calculate(full_effects,draws) %>% lapply(function(x) tail(x,5000) %>% variable_summaries)
...
output_list %>% saveRDS(file = "models/model_D.RDS")