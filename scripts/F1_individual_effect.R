...
interference_idxs <- interference_list %>% 
  ...
  ungroup() %>%
  mutate(ind_nonredundant = ind %>% as.factor %>% as.numeric)
...
# An effect per individual that affects all clones equally 
b_individual <- normal(0,0.05,dim = c(1,max(interference_idxs$ind_nonredundant)))
...
r <- (self_term + b_individual[interference_idxs$ind_nonredundant]) * (age[interference_idxs$ind_age]) + offset_per_individual
mu <- ilogit(r)
