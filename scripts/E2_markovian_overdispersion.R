...
X_sparse <- t(train_subset$counts[gene_idxs,] + 1)[cbind(interference_idxs$ind_age,interference_idxs$site)] %>%
  as.matrix(ncol = 1) %>%
  as_data
coverage_sparse <- t(train_subset$coverage[gene_idxs,] + 1)[cbind(interference_idxs$ind_age,interference_idxs$site)] %>%
  as.matrix(ncol = 1) %>%
  as_data
ind_o <- sapply(c(1:nrow(interference_idxs)),function(x) {
  ...
  M %>%
    as.numeric %>%
    return})
ind_previous_tp <- sapply(interference_idxs$index,function(x) {
  M <- rep(0,nrow(interference_idxs))
  if (interference_idxs$previous_timepoint_index[x] != 0) {
    M[interference_idxs$previous_timepoint_index[x]] <- 1
  }
  M %>%
    as.numeric %>%
    return
})
ind_first_tp <- sapply(interference_idxs$index,function(x) {
  M <- rep(0,nrow(interference_idxs))
  if (interference_idxs$first_timepoint[x] != 0) {
    M[x] <- 1
  }
  M %>%
    as.numeric %>%
...
ind_non_first_tp <- sapply(interference_idxs$index,function(x) {
  M <- rep(0,nrow(interference_idxs))
  if (interference_idxs$first_timepoint[x] == 0) {
    M[x] <- 1
  }
...
GN <- 250000 #variable(lower = 0) # Basically this is the number of generations
overdispersion <- 250
...
#mu <- matrix(seq(0.2,0.6,length.out = length(mu)),nrow = nrow(mu),ncol = ncol(mu))

mu_tm1 <- t(t(mu) %*% ind_previous_tp) + (ind_first_tp %*% matrix(0.5,nrow = nrow(mu),ncol = ncol(mu)))
delta_age <- t(age[interference_idxs$ind_age] - (t(matrix(age[interference_idxs$ind_age],ncol = 1)) %*% ind_previous_tp))
delta_age[which.min(delta_age)] <- 1
markovian_variance <- mu_tm1 * (1 - mu_tm1) * (1 - (1 - delta_age/(2 * GN)))
alpha_over_1 <- ((mu * (1 - mu))/markovian_variance - 1) * mu
beta_over_1 <- ((mu * (1 - mu))/markovian_variance - 1) * (1 - mu)
mu_over_1 <- beta(alpha_over_1 + 1,
                  beta_over_1 + 1)
combined_mu <- (ind_first_tp %*% mu) + t(t(mu) %*% mu_over_1)
alpha_full <- 0.5 * combined_mu * overdispersion
beta_full <- (1 - 0.5 * combined_mu) * overdispersion

distribution(X_sparse) <- beta_binomial(size = coverage_sparse + 1,
                                        alpha = alpha_full + 1,
                                        beta = beta_full + 1)
### 
