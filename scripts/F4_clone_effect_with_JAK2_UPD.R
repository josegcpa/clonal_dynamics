...
JAK2_individuals <- (train_subset$site_to_individual_indicator %*% t(train_subset$individual_indicator))[
  which(train_subset$site_multiple_to_site_indicator[,site_list == 'JAK2-V617F']!=0),] %>%
  as.logical() %>%
  as.numeric()
n_JAK2_mutations <- sum(JAK2_individuals)

min_max_ages <- train_subset$individual_indicator %>% 
  apply(1,function(x) x * train_subset$ages) %>% 
  t %>% 
  apply(1,function(x) c(min(x[x!=0]),max(x))) %>%
  t %>% 
  as.data.frame()
colnames(min_max_ages) <- c("min","max")
...
b_clone <- normal(mean = b_clone_mean,
                  sd = b_clone_sd,
                  dim = c(1,length(unique(interference_idxs$ind_site))))

### Sparse model

X_sparse <- t(train_subset$counts[gene_idxs,])[interference_idxs$ind_age] %>%
  as.matrix(ncol = 1) 
coverage_sparse <- t(train_subset$coverage[gene_idxs,])[interference_idxs$ind_age] %>%
  as.matrix(ncol = 1) 
...
age_at_upd <- do.call(
  rbind,
  lapply(
    seq(1,nrow(min_max_ages)),
    function(x) variable(min_max_ages$min[x]-min_age,
                         min_max_ages$max[x]-min_age,
                         dim = 1))
)
delta_g <- normal(0,0.02,truncation = c(0,Inf))
JAK2_ind_idx <- which(JAK2_individuals == 1)
JAK2_idx <- which(train_subset$unique_site[gene_idxs]=='JAK2-V617F')
JAK2_mask <- interference_idxs %>%
  apply(
    1,FUN = function(x) {
      site <- x[1]
      ind <- x[3]
      if (site %in% JAK2_idx & ind %in% JAK2_ind_idx) {
        return(which(JAK2_ind_idx == ind))
      } else {
        return(0)
      }
    }
  )
JAK2_mask_greta <- as_data(rep(100,nrow(interference_idxs)))
JAK2_mask_greta[as.logical(JAK2_mask)] <- age_at_upd[JAK2_mask]
upd_event <- age[interference_idxs$ind_age] > JAK2_mask_greta
upd_increase_growth_effect <- upd_event * delta_g

self_term <- full_effects[interference_idxs$site]
offset_per_individual <- u[interference_idxs$ind_site]
r <- (
  self_term + upd_increase_growth_effect + b_clone[interference_idxs$ind_site]
  ) * (age[interference_idxs$ind_age]) + offset_per_individual
mu <- 0.5 / (1 + exp(-r))
