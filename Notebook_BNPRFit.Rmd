...
if (file.exists(file_name) == T) {
  ...
  all_estimates_trees <- mclapply(
    trees,
    mc.cores = 2, # 2 cores led to OOM errors
    mc.cleanup = T,
    function(x) {
...
```{r}
all_estimates_trimmed_5_years <- all_estimates
...
```{r}
all_estimates_trimmed_10_years <- all_estimates
par(mfrow = c(4,5),mar = c(3,1,1,1))
      ...
      if (length(all_estimates[[x]]) > 0) {
        lapply(1:length(all_estimates_trimmed_5_years[[x]]),function(y) {
          ...
          if (!is.null(nle) & class(nle) != "try-error") {
            ...
            tao <- all_estimates_trimmed_5_years[[x]][[y]]$mutation_timing
      ...
      if (length(all_estimates[[x]]) > 0) {
        lapply(1:length(all_estimates_trimmed_10_years[[x]]),function(y) {
          ...
          if (!is.null(nle) & class(nle) != "try-error") {
            ...
            tao <- all_estimates_trimmed_10_years[[x]][[y]]$mutation_timing
...
set.seed(777)
S <- sample(simulated_trajectories_df$id,
                  size = 15,replace = F)

ggplot(data = NULL) +
  geom_hline(yintercept = 2e5,size = 0.25,colour = "goldenrod",alpha = 0.7) +
  geom_line(data = subset(simulated_trajectories_df,id %in% S),
            aes(x = Gen/100,y = Count,colour = "Simulated",
                group = paste(MutID_C,driver)),
            size = 0.25,
            alpha = 0.7) + 
  geom_line(data = subset(bnpr_estimates_df,id %in% S),
            aes(x = time/100,y = transformed_y,
                group = paste(clade,driver),colour = "BNPR estimate"),
            size = 0.25,
            alpha = 0.5) + 
  facet_wrap(~ id + driver,scales = "free") + 
  theme_gerstung(base_size = 6) + 
  theme(strip.text = element_blank(),legend.position = "bottom") + 
  scale_y_continuous(trans = 'log10') + 
  coord_cartesian(ylim = c(NA,2e8),c(0,8)) +
  scale_colour_manual(values = c(Simulated = "grey50",`BNPR estimate` = "orchid"),name = NULL) + 
  xlab("Generation (x100)") + 
  ggsave("figures/simulations/trajectories_wf_bnpr_subset.pdf",height = 3,width = 4)
```

## Sensitivity analysis - normal vs. trimmed

Here we see if using trimmed leads to very different estimates. Trimming the data by 5 or even 10 years does not lead to very different estimations for the late growth rate as we propose here. Indeed, the average difference we can expect between trimmed-by-10-years and non-trimmed data is 0.5% growth per generation for late growth estimates and 0.3% growth per generation for early growth estimates.
...
diff_fits <- rbind(
  all_bnpr_fits %>%
    select(name,clade,early = cp_1,late = cp_late,earliest,latest) %>%
...
rbind(
  ...
  ungroup %>% 
  summarise(
    `Original vs. 5 years` = mean(abs(`Original trajectory` - `Trimmed by 5 years`),na.rm = T),
    `Original vs. 10 years` = mean(abs(`Original trajectory` - `Trimmed by 10 years`),na.rm = T),
    `5 years vs. 10 years` = mean(abs(`Trimmed by 5 years` - `Trimmed by 10 years`),na.rm = T)) %>%
...
bnpr_wf_merge_df %>% 
  ...
  ggplot(aes(x = ifelse(expected_late_ratio_wf < 0.8,"Saturating","Near constant"),y = expected_late_ratio)) +
...
asymptote_graph <- bnpr_wf_merge_sigmoid_df %>%
  ggplot(aes(x = 2e5,y = nl_asym,colour = col)) + 
  geom_point(size = 0.5) + 
  scale_y_continuous(trans = 'log10') +
  scale_x_continuous(trans = 'log10') +
  theme_gerstung(base_size = 6) +
