...
tree_subsets <- trees[c(3,45,68,88)]
all_trees <- list()
for (tree_name in names(tree_subsets)) {
  fitness <- str_match(tree_name,"[0-9.]+")
  tree_ <- tree_subsets[[tree_name]]
  tree_ultra <- tree_$tree_ultra
  driver_list <- tree_$driver_list
  clades_plot <- list()
  for (driver in names(driver_list)) {
    clades_plot[[driver]] <- which(tree_$tree$S %in% driver_list[[driver]])
  }
  tree_groups <- groupOTU(tree_ultra,clades_plot) 
  all_trees[[tree_name]] <- ggtree(tree_groups,aes(colour = group!='0')) + 
    scale_color_jama(name=NULL,labels = c("Comb","Expansion")) + 
    ggtitle(sprintf("s=%s",fitness)) + 
...
plot_grid(plotlist = lapply(all_trees,function(x) x + theme(legend.position = 'none')),nrow=1) %>%
  plot_grid(get_legend(all_trees[[3]]),rel_heights = c(1,0.1),ncol = 1) + 
  ggsave("figures/simulations/tree_examples.pdf",height=2,width=4)
```
...
if (file.exists(file_name) == T) {
  ...
  for (obj_name in names(trees)) {
    ...
    Tree <- list(tree = obj$tree_ultra)
    Tree$tree$tip.label <- obj$tree$S
    Tree$tree <- trim_tree(Tree$tree,l = 100)
    Tree$tree$edge.length[is.infinite(Tree$tree$edge.length)] <- 0
    tree <- Tree$tree
    tree_ <- tree
    tree_$edge.length <- tree_$edge.length / 800
...
plot_tree_list <- list()
plot_list <- list()
for (b in list(all_estimates[[110]][[2]],all_estimates[[115]][[2]])) {
  ll <- predict(object = b$linear_estimate,newdata = data.frame(X = b$data$X))
  sig <- predict(b$non_linear_estimate,b$data$X)
  biph_par <- b$change_point_regression$par
  biph <- change_point(b$data$X,biph_par[1],biph_par[2],
                       biph_par[3],biph_par[4])
  
  n_tips <- length(b$clade)
  
  plot_tree_list[[length(plot_tree_list)+1]] <- b$tree %>%
    keep.tip(b$clade) %>% 
    multi2di() %>%
    ggtree +
  ...
  plot_list[[length(plot_list)+1]] <- data.frame(
    Y=b$data$Y_exp,ll=exp(ll),sig=sig,bi=exp(biph),
    Ymin=exp(b$data$Y025),Ymax=exp(b$data$Y975),X=b$data$X) %>%
    ggplot(aes(x = X)) + 
    geom_ribbon(aes(ymin = Ymin,ymax = Ymax,fill = "BNPR"),alpha = 0.2) +
    geom_line(aes(y = Y,colour = "BNPR"),size = 1,alpha = 0.5) + 
    geom_line(aes(y = ll,colour = "Log-linear"),linetype=2) + 
    geom_line(aes(y = sig,colour = "Sigmoid"),linetype=2) +
    geom_line(aes(y = bi,colour = "Biphasic"),linetype=2) + 
    theme_gerstung(base_size = 6) +
    scale_y_continuous(trans = 'log10') + 
    theme(legend.position="bottom",legend.key.height=unit(0,"cm")) +
    scale_colour_d3(breaks = c("BNPR","Log-linear","Sigmoid","Biphasic"),
                    limits = c("BNPR","Log-linear","Sigmoid","Biphasic"),
                    name=NULL) + 
    scale_fill_d3(breaks = c("BNPR","Log-linear","Sigmoid","Biphasic"),
                  limits = c("BNPR","Log-linear","Sigmoid","Biphasic"),
                  name=NULL) + 
    xlab("Generations") + 
    ylab("Neff") + 
    coord_cartesian(ylim = c(10,1e8)) + 
    ggtitle(sprintf("No. of tips in clade: %s",n_tips)) + 
    guides(colour = guide_legend(nrow = 2))
}

plot_grid(
  plot_grid(
    plot_tree_list[[2]],plot_tree_list[[1]],
    plot_list[[2]] + theme(legend.position = "none"),
    plot_list[[1]] + theme(legend.position = "none"),
    ncol = 2,align = "v",axis="tb",
    rel_heights = c(0.5,1)
  ),
  get_legend(plot_list[[1]]),rel_heights = c(1,0.2),ncol=1
) + 
  ggsave("figures/simulations/bnpr-fit-examples.pdf",height=2,width=3)
```
...
rbind(trajectories_trimmed_0,
  ...
  subset(paste(id,clade) %in% sample(unique(paste(id,clade)),20,replace = F)) %>% 
  ...
  ggsave("figures/simulations/trimmed_representation.pdf",height = 2.5,width = 3.5)
```
...
fitness_fits <- list()
for (f in unique(plot_data$fitness)) {
  tmp <- plot_data[plot_data$fitness == f,] %>%
    arrange(x) %>%
    subset(x < 400)
  min_Y <- mean(tmp$y[tmp$x == 0])
  tmp$y <- tmp$y/min_Y
  fitness_fits[[f]] <- lm(log(y) ~ x-1,tmp,weights = 1/tmp$w)
  fitness_fits[[f]]$min_Y <- min_Y
}

fitness_fits_df <- names(fitness_fits) %>%
  lapply(function(x) {
    X <- seq(0,max(plot_data$x))
    min_Y <- fitness_fits[[x]]$min_Y
    C <- fitness_fits[[x]]$coefficients
    data.frame(x = X,y = min_Y * exp(C[1] * X),
               fitness = x)
  }) %>%
  do.call(what = rbind)
...
plot_data %>% 
  ...
  geom_line(data = fitness_fits_df,colour = "red",alpha = 1) +
  facet_wrap(~ fitness) + 
...
set.seed(42)
all_bnpr_estimates_df <- do.call(rbind,bnpr_estimates_) %>% 
  mutate(w = (log(quant0.975) - log(quant0.025))^2/16) %>% 
  group_by(id, clade) %>% 
  mutate(w_sum = mean(w))
...
high_var_ids <- all_bnpr_estimates_df %>% 
  subset(w_sum > 5) %>%
  group_by(id,clade) %>%
  filter(all(mean < 1e9)) %>%
  subset(paste(id,clade) %in% sample(paste(id,clade),n_examples)) %>% 
  ungroup %>%
  arrange(w_sum) %>% 
  select(id,clade) %>% 
  distinct

low_var_ids <- all_bnpr_estimates_df %>% 
  subset(w_sum < 5) %>%
  group_by(id,clade) %>%
  filter(all(mean < 1e9)) %>%
  subset(paste(id,clade) %in% sample(paste(id,clade),n_examples)) %>% 
...
plot_tree_list <- list()
plot_list <- list()
low_var_estimates <- apply(low_var_ids,1,function(x) {
  X <- all_estimates[[x[1]]][[as.numeric(x[2])]]
  
  X$true_count <- all_driver_fits[[x[1]]][[x[2]]]$data %>% 
    ungroup %>% 
    subset(Gen == max(Gen))
  return(X)
})
for (b in low_var_estimates) {
  ll <- predict(object = b$linear_estimate,newdata = data.frame(X = b$data$X))
  n_tips <- length(b$clade)
  ...
  plot_list[[length(plot_list)+1]] <- data.frame(
    ...
    geom_point(data = b$true_count,aes(x = Gen,y = Count),inherit.aes = F) +
    theme_gerstung(base_size = 6) +
    scale_y_continuous(trans = 'log10') + 
    theme(legend.position="bottom",legend.key.height=unit(0,"cm")) +
    xlab("Generations") + 
    ylab("Neff") + 
    coord_cartesian(ylim = c(10,1e8)) + 
    ggtitle(sprintf("No. of tips in clade: %s\nAverage variance: %.2f",n_tips,mean(b$data$W,na.rm=T))) + 
    guides(colour = guide_legend(nrow = 2))
}

plot_grid(
  plot_tree_list[[1]],plot_tree_list[[2]],plot_tree_list[[3]],plot_tree_list[[4]],
  plot_list[[1]] + theme(legend.position = "none"),
  plot_list[[2]] + theme(legend.position = "none"),
  ...
  rel_heights = c(0.8,1)) + 
  ggsave("figures/simulations/low_var_trajectory_examples.pdf",height = 2,width = 5)

plot_tree_list <- list()
plot_list <- list()
high_var_estimates <- apply(high_var_ids,1,function(x) {
  X <- all_estimates[[x[1]]][[as.numeric(x[2])]]
...
plot_grid(
  ...
  nrow = 2,align = "v",axis="tb",
  rel_heights = c(0.8,1)) + 
  ggsave("figures/simulations/high_var_trajectory_examples.pdf",height = 2,width = 5)
```
...
bnpr_wf_merge_df %>%
  group_by(col) %>%
  summarise(m = min(clade_size),median = median(clade_size),M = max(clade_size))
```
...
plot_grid(
  ...
  ncol = 2) +
  ggsave("figures/simulations/validation_BNPR_smaller.pdf",height = 3.9,width = 3,useDingbats=FALSE)
