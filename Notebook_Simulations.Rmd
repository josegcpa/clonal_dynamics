...
1:length(all_data$N200$draws) %>%
  lapply(function(x) data.frame(as.matrix(all_data$N200$draws[[x]]),
                                chain=x,
                                idx=1:nrow(all_data$N200$draws[[x]]))) %>%
  do.call(what = rbind) %>% 
  gather(key = 'key',value = 'value',-chain,-idx) %>% 
  subset(grepl('genetic_coef',key)) %>% 
  mutate(key = str_match(key,'1\\.[0-9]+')) %>%
  mutate(key = as.numeric(str_match(key,'[0-9]+$'))/1000) %>% 
  ggplot(aes(x = idx,y = value,colour = as.factor(chain))) + 
  geom_line(size = 0.25,alpha = 0.8) + 
  facet_wrap(~ reorder(sprintf('s=%s',key),key),scales = "free_y") + 
  xlab("Sample index") + 
  ylab("Value") +
  theme_gerstung(base_size = 6) + 
  scale_color_manual(values = paste0("skyblue",c(4,3,2,1)),name = "Chain") + 
  theme(legend.key.height = unit(0,"cm")) + 
  scale_x_continuous(breaks = c(0,500,1000)) +
  ggsave('figures/simulations/chains-genetic.pdf',height = 3.5,width = 5)
```

```{r diagnostics_200k_2}
rg_stat <- coda::gelman.diag(all_data$N200$draws)
ess <- coda::effectiveSize(all_data$N200$draws)
```

```{r diagnostics_200k_2_plot,fig.height=1.3,fig.width=3}
rg_stat_df <- data.frame(rg = rg_stat$psrf[,1],idx = 1:length(rg_stat$psrf[,1]),ess = ess)
rg_stat_df$var_type <- c(genetic='Genetic',clone='Unknown-cause',individual='Offset',beta='Beta')[
  str_match(rownames(rg_stat_df),'[a-z]+')]
rg_stat_plot <- rg_stat_df %>%
  ggplot(aes(x = idx,y = rg,colour = var_type)) +
...
ess_plot <- rg_stat_df %>%
  ggplot(aes(x = idx,y = ess,colour = var_type)) +
  geom_point(size = 0.5,alpha = 0.6) +
  theme_gerstung(base_size = 6) + 
  xlab("Variable index") + 
  ylab("Effective sample size") + 
  scale_color_aaas(name = NULL,breaks = c('Genetic','Unknown-cause','Offset','Beta')) + 
  theme(legend.position = 'bottom',
        legend.key.size = unit(0,"cm"))

plot_grid(
  plot_grid(rg_stat_plot + theme(legend.position = 'none'),
            ess_plot + theme(legend.position = 'none')),
  get_legend(ess_plot),ncol = 1,rel_heights = c(1,0.1)) + 
  ggsave('figures/simulations/rg-ess.pdf',height = 1.3,width=3)
```
...
To assess the goodness of fit we calculate, for each timepoint, the likelihood under our model. If the tail probability of that point under our model is not contained within the 95\% confidence interval of the model, we consider this to be an outlier; whenever a trajectory has at least one outlier, we discard it and do not analyse it any further. Unless noted otherwise, we here use only the simulations with N = 200,000 and g = 13.

```{r calculate_gof,fig.height=3,fig.width=3}
get_draws <- function(D,individual_factor,genetic_factor) {
  c_names <- colnames(D)
  c_list <- list(genetic=grep('genetic',c_names),clone_specific=grep('clone_specific',c_names),
                 offset=grep('offset',c_names),beta=grep('beta',c_names))
  output <- list(genetic = D[,c_list$genetic[genetic_factor]],
                 clone_specific = D[,c_list$clone_specific[individual_factor]],
                 offset = D[,c_list$offset[individual_factor]],
                 beta = D[,c_list$beta[1]])
  return(output)
}
...
get_r_values <- function(draws,Data,Age,gen_time) {
  ...
  traj_estimates <- data.frame(q05=rep(0,nrow(Data)),q50=rep(0,nrow(Data)),
                               q95=rep(0,nrow(Data)),mean=rep(0,nrow(Data)))
  for (i in 1:nrow(Data)) {
    tmp <- Data[i,]
    o <- get_draws(draws[[1]],tmp$individual_factor,tmp$fitness_factor)
    x <- (o$genetic + o$clone_specific) * (tmp$Gen / gen_time-min_age) + o$offset
    p <- inv.logit(x)/2
    alpha_values <- (p * o$beta)/(1-p)
    S <- rbbinom(1000,tmp$coverage,alpha = alpha_values,o$beta)
    traj_estimates[i,] <- c(quantile(S,c(0.05,0.5,0.95),na.rm=T),mean(S,na.rm=T))
    }
  ...
  r_values <- data.frame(
    ...
    Mutation_C = Data$Mutation_C,
    Individual = Data$Individual,
    min_age = min_age,
    q05 = traj_estimates$q05,
    q50 = traj_estimates$q50,
    q95 = traj_estimates$q95) %>%
    as_tibble() %>% 
...
```{r explained_variants_plot,fig.height=1.4,fig.width=2}
explained_variants_plot <- gof_plot_values %>%
  ...
  xlab("Fitness") +
  coord_flip() +
  theme(legend.key.size = unit(0.2,"cm"),plot.margin = margin(4,8,4,4),
        legend.margin = margin())

explained_variants_plot + 
  ggsave('figures/simulations/explained-trajectories.pdf',height = 1.4,width = 2)
```
...
```{r trajectory_examples,fig.height=2,fig.width=2.5}
min_data <- min(all_data$N200$min_age_data)
r_values_N200 %>%
  subset(fitness %in% c(0.002,0.010,0.015,0.020)) %>%
  group_by(fitness) %>%
  mutate(average_age = mean(age)) %>% 
  group_by(fitness,individual_factor) %>%
  mutate(d = mean(age)-average_age) %>% 
  group_by(fitness) %>%
  filter(d %in% c(range(d),d[which.min(abs(d))])) %>%
  group_by(d,fitness) %>%
  filter(individual_factor %in% sample(individual_factor,1)) %>% 
  ggplot(aes(x = age + min_data,y = true_count/coverage)) + 
  geom_ribbon(aes(group = individual_factor,ymin = q05/coverage,ymax = q95/coverage),alpha = 0.3) +
  geom_point(size = 0.5) + 
  geom_line(aes(group = individual_factor),linetype=3,size = 0.25) +
  geom_line(aes(y = q50/coverage, group = individual_factor)) +
  facet_wrap(~ fitness,ncol=2,scales = 'free_x') + 
  scale_y_continuous(trans = 'log10') + 
  theme_gerstung(base_size = 6) + 
  ylab("VAF") +
  xlab("Age") + 
  ggsave('figures/simulations/example-trajectories.pdf',height = 2,width = 2.5)
```
...
age_quantile_data <- age_data %>% 
  mutate(true_clone_age = 5 * floor(true_clone_age/5)) %>%
  group_by(true_clone_age) %>% 
  summarise(ymin = quantile(Q50,0.25),
            ymax = quantile(Q50,0.75),
            y = quantile(Q50,0.5)) %>%
  arrange(true_clone_age) %>%
  mutate(ymin = ma(ymin),y = ma(y),ymax = ma(ymax))
...
age_data %>% 
  ...
  geom_linerange(aes(x = true_clone_age,
                 ...
                 size = 0.2,
                 alpha = 0.8) + 
  geom_smooth(data = age_quantile_data,aes(y = y),colour = "green4",se=F) + 
  geom_smooth(data = age_quantile_data,aes(y = ymin),colour = "red4",se=F) + 
  geom_smooth(data = age_quantile_data,aes(y = ymax),colour = "red4",se=F) + 
  scale_color_gradient(low = "pink",high = "red4",name = "Fitness") + 
  theme_gerstung(base_size = 6) + 
  ylab("Inferred age\nat clone foundation") +
  xlab("Simulated age\nat clone foundation") + 
  ...
  coord_cartesian(ylim = plot_min_max) +
  scale_shape_discrete(name = "Generation time, population size") + 
  ggsave(filename = "figures/simulations/estimated_ages_plot_bands.pdf",height = 3,width = 3)
```
...
```{r age_residuals,fig.height=2,fig.width=1.7}
...
ggplot(age_data_post,aes(x = fitness,y = residuals)) +
  ...
  geom_point(size = 0.5) +
  geom_boxplot(aes(group = cut(fitness,c(0,0.005,0.010,0.015,0.020,Inf))),
               alpha = 0.8,outlier.alpha = 0,size = 0.25) + 
  theme_gerstung(base_size = 6) + 
  ylab("Age at onset residuals") +
  xlab("Simulated fitness") + 
  ggsave('figures/simulations/fitness-vs-inferred-age-residuals.pdf',height = 2,width = 1.7)

age_data_post %>%
  mutate(fitness_split = fitness <= 0.01) %>%
  group_by(fitness_split) %>%
  summarise(mean(abs(residuals)))
```

The estimate for the ages, while not as good as the estimate for the coefficients, appears to be quite close to the true value. Additionally, on average, there is a difference of ~9 years between our estimates and the true values. Residuals do not appear to be normally distributed, but a wider spread is observable for lower ages at clone foundation as per the figure above. Below we will observe that this is mostly caused by an unexpectedly high amount of time spent during the stochastic growth regime. 

```{r compare_population_sizes_gen_time,fig.width=6,fig.height=7}
age_data_full %>%
  group_by(Individual,true_clone_age,population,gen_time) %>%
...
```{r compare_pop_sizes_gen_time_heatmap,fig.height=2,fig.width=3}
age_data_full %>%
  group_by(population,gen_time) %>%
  ...
  geom_tile(size = 0.5) + 
  geom_tile(aes(colour = Y),size = 0.5,fill=NA) + 
  geom_label(size = 2,label.size = unit(0,"cm"),
             label.r = unit(0,"cm"),alpha = 0.8,fill = "white") + 
  theme_gerstung(base_size = 6) + 
  scale_fill_material(palette = "teal",name = "Mean absolute\nerror for age\nat onset") + 
  ...
  ggsave("figures/simulations/age-at-onset-mae.pdf",height=2,width=3)
```
...
```{r real_age_distribution,fig.height=1.2,fig.width=3.5}
dir.create(path = "figures/rank_analysis",showWarnings = F)
age_density_plot_50 <- all_data$N50$Data %>% 
  select(GenAtCloneFoundation,Mutation_C,fitness) %>%
  distinct() %>% 
  mutate(f = ifelse(fitness > 0.15,'>15% annual growth','<15% annual growth')) %>% 
  ggplot(aes(x = GenAtCloneFoundation,colour = f)) +
  geom_density() + 
  geom_density(aes(colour = 'All clones'),linetype = 2) +
  xlab("Age at onset") +
  theme_gerstung(base_size = 6) + 
  scale_colour_d3(name = NULL) +
  ggtitle("N = 50,000; g = 1")
age_density_plot_100 <- all_data$N100$Data %>% 
  select(GenAtCloneFoundation,Mutation_C,fitness) %>%
  distinct() %>% 
  mutate(f = ifelse(fitness * 5 > 0.15,'>15% annual growth','<15% annual growth')) %>% 
  ggplot(aes(x = GenAtCloneFoundation / 5,colour = f)) +
  geom_density() + 
  geom_density(aes(colour = 'All clones'),linetype = 2) +
  xlab("Age at onset") +
  theme_gerstung(base_size = 6) + 
  scale_colour_d3(name = NULL) +
  ggtitle("N = 100,000; g = 5")
age_density_plot_200 <- all_data$N200$Data %>% 
  select(GenAtCloneFoundation,Mutation_C,fitness) %>%
  distinct() %>% 
  mutate(f = ifelse(fitness * 13 > 0.15,'>15% annual growth','<15% annual growth')) %>% 
  ggplot(aes(x = GenAtCloneFoundation / 13,colour = f)) +
  geom_density() + 
  geom_density(aes(colour = 'All clones'),linetype = 2) +
  xlab("Age at onset") +
  theme_gerstung(base_size = 6) + 
  scale_colour_d3(name = NULL) +
  ggtitle("N = 200,000; g = 13")

plot_grid(age_density_plot_50 + theme(legend.position = 'none') + coord_cartesian(ylim = c(0,0.03)),
          age_density_plot_100 + theme(legend.position = 'none') + coord_cartesian(ylim = c(0,0.03)),
          age_density_plot_200 + theme(legend.position = 'none') + coord_cartesian(ylim = c(0,0.03)),
  nrow = 1) %>%
  plot_grid(get_legend(age_density_plot_200 + 
                         theme(legend.position='bottom',legend.key.size = unit(0.05,"cm"))),
            rel_heights = c(1.2,0.2),ncol=1) +
  ggsave("figures/rank_analysis/distributions_real.pdf",height = 1.2,width = 3.5)
```
...
age_at_onset_distribution <- list(
  g13 = dist_subset$DistToFirstTrue[dist_subset$true_pop_size == 200e3],
  g5 = dist_subset$DistToFirstTrue[dist_subset$true_pop_size == 100e3],
  g1 = dist_subset$DistToFirstTrue[dist_subset$true_pop_size == 50e3]) %>%
  lapply(function(x) {
    x <- ran(x)
    x <- x[!is.na(x)]
    return(fitdistrplus::fitdist(x,"beta",method = "mge"))
  })

dist_estimate_data <- age_at_onset_distribution %>%
  lapply(function(x) {
    a <- x$estimate[1]
    b <- x$estimate[2]
    X <- seq(0,1,length.out = 1000)
    list(
      data = data.frame(values = x$data),
      estimation = data.frame(x = X,d = dbeta(X,a,b))) %>%
      return
  })
...
plot_list <- dist_estimate_data %>% 
  lapply(function(x) {
    ggplot(x$data,aes(x = values)) + 
      geom_histogram(aes(y = ..density..),bins=10,fill=NA,size = 0.25,colour = 'black') + 
      geom_line(data = x$estimation,aes(x = x,y = d),colour = "red4") + 
      theme_gerstung(base_size = 6) + 
      xlab("Data") + 
      ylab("Density") + 
...
plot_grid(
  plot_list[[1]] + ggtitle("N=200,000; g=13"),
  plot_list[[2]] + ggtitle("N=100,000; g=5"),
  plot_list[[3]] + ggtitle("N=50,000; g=1"),
  nrow = 1) + 
  ggsave("figures/rank_analysis/beta_distance_fits.pdf",height = 1,width = 3.5)
```
...
chisq_statistic <- distances_age_at_onset %>%
  ...
  summarise(
    ...
    ks_beta = suppressWarnings(ks.test(ran(DistToFirst),"pbeta",
                                       age_at_onset_distribution[[paste0('g',true_gen_time)[1]]]$estimate[1],
                                       age_at_onset_distribution[[paste0('g',true_gen_time)[1]]]$estimate[2])$statistic),
    HE = hist_entropy(ran(DistToFirst),M=1)) 

plot_stat_data <- chisq_statistic %>% 
  group_by(true_pop_size,true_gen_time) %>% 
  mutate(R = rank(ks_beta,ties.method = "min")) %>%
  mutate(R = as.numeric(as.factor(R))) %>% 
  ungroup %>% 
  mutate(Y = (gen_time == true_gen_time) & (population == true_pop_size)) 

tps <- c(50e3,100e3,200e3)
gt <- c(1,5,13)

plot_list <- 1:3 %>%
  lapply(function(x) {
    plot_stat_data %>% 
      subset(true_pop_size == tps[x]) %>%
      ggplot(aes(x = as.factor(gen_time),y = as.factor(population),fill = ks_beta)) +
      geom_tile(colour = NA) +
      geom_tile(aes(colour = Y),size = 0.25,fill = NA) +
      geom_text(aes(label = R),size = 2.3) +
      xlab("Generations/year") +
      ylab("Population size") +
      theme_gerstung(base_size = 6) +
      scale_fill_material(palette = "red",name = "KS\nstatistic",limits = c(0,0.6)) +
      scale_x_discrete(expand = c(0,0)) +
      scale_y_discrete(expand = c(0,0)) +
      ...
      ggtitle(sprintf("N = %s; Tg = %s",tps[x],gt[x]))
  })

plot_grid(plotlist = lapply(plot_list,function(x) x + theme(legend.position = 'none')),ncol = 3) %>%
  plot_grid(get_legend(plot_list[[1]]),rel_widths = c(1,0.1),nrow=1) +
  ggsave("figures/rank_analysis/statistical_distance_d.pdf",height = 1,width = 5)
```
...
```{r inspecting_bad_ones, fig.width=4.5,fig.height=2}
...
file_ids$Mutation_C <- individuals_mutations$Mutation_C
file_ids$bad <- T

G <- 5

bad_predictions_list <- file_ids %>%
  select(-Mutation,Mutation_C) %>%
  apply(1,function(x) {
    x <- as.numeric(x)
    if (x[2] < 1) {
...
bad_predictions <- bad_predictions_list %>%
  lapply(function(x) {
    fi <- as.numeric(x$fitness[1])
    mr <- as.numeric(x$mutation_rate[1])
    tmp <- file_ids %>%
      subset(as.numeric(as.character(mutation_rate)) == mr &
               as.numeric(as.character(fitness)) == fi)
    x %>%
      mutate(bad = Mutation_C %in% tmp$Mutation_C) %>%
      return}) %>%
  do.call(what = rbind) %>% 
  filter(Mutation > 0) %>%
  group_by(Mutation,Gen,fitness,mutation_rate,replicate,Mutation_C,bad) %>%
...
bad_predictions %>% 
  ...
  filter(all(length(Count[Driver==T & bad == T] > 200) > 190)) %>%
  mutate(bad = ifelse(bad==T,'Error > 20 years','Not inferred/error < 20 years')) %>%
  mutate(fitness = as.numeric(as.character(fitness))) %>%
  mutate(mutation_rate = as.numeric(as.character(mutation_rate))) %>%
  ggplot(aes(x = Gen/13,y = Count,color = Driver,alpha=Alpha,
             group = paste(Driver,bad,Mutation_C,replicate),
             size = as.numeric(Driver))) + 
  geom_hline(aes(yintercept = (1 - (1+fitness)^-1) / (1 - (1+fitness)^-pop_size) * pop_size),
             size = 0.25) +
  geom_line(aes(linetype = bad),size = 0.25) + 
  ...
  scale_linetype_manual(name = NULL,values = c(1,3)) +
  theme(legend.position = "bottom",strip.text = element_text(margin = margin()),
        legend.key.height = unit(0,"cm"),legend.key.width = unit(0.2,"cm"),legend.spacing = unit(0,"cm")) + 
  ...
  ylab("Count") + 
  xlab("Age") + 
  facet_wrap(~ reorder(sprintf('s=%s; mu=%s',fitness,mutation_rate * (10e-9)),
                       fitness*10000 + mutation_rate),
             nrow = 2) + 
  ggsave("figures/simulations/examples-bad-inference.pdf",height=2,width=4)
```
...
ggsave(estimated_ages_plot + theme(legend.position = "right"),
       filename = "figures/simulations/estimated_ages_plot.pdf",height = 3,width = 3)
```
