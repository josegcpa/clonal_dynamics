...
age_data_full %>% 
  mutate(pre_birth = Q50 < 0) %>% 
  group_by(gen_time,population) %>% 
  summarise(P = sum(pre_birth) / length(pre_birth)) %>% 
  ggplot(aes(x = as.factor(population),y = as.factor(gen_time),fill = P)) +
  geom_tile() +
  geom_text(aes(label = round(P,4))) + 
  theme_gerstung(base_size = 6) + 
  ylab("tau (gen/year)") + 
