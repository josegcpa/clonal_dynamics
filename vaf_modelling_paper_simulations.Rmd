...
author: José Guilherme de Almeida & Moritz Gerstung
output: 
  html_document:
    theme: lumen
    highlight: tango
...
The advantage of these simulations is that we have a very precise value for when the mutation first appeared. This allows us to assess how closely we can estimate the age at clone foundation. We do a few filtering steps to ensure that we only consider estimates where we are as confident as possible - as such we:

1. Only use trajectories with a residual effect below 0.7 (explained trajectories) 
2. Exclude median estimates below -20 years and above 125 years 
3. Exclude estimates which are not contained between $[-200,200]$ 
...
age_data_list <- lapply(list(c(2,5e4),c(13,5e4),c(2,2e5),c(13,2e5)),function(x) {
  ...
  r_values %>% 
    ...
    select(fitness,
           ...
           true_clone_age,Individual,Mutation,age) %>%
    distinct() %>%
    mutate(gen_time = gen_time,
           population = population) %>%
    mutate(estimated_clone_age = ifelse(
      genetic_effect + clone_effect > 0,t0_adjusted(individual_offset,genetic_effect + clone_effect,gen_time,population)+min_age,NA),
           estimated_clone_age_005 = ifelse(
      genetic_effect_005 + clone_effect_005 > 0,t0_adjusted(individual_offset_005,genetic_effect_005 + clone_effect_005,gen_time,population)+min_age,NA),
           estimated_clone_age_095 = ifelse(
      genetic_effect_095 + clone_effect_095 > 0,t0_adjusted(individual_offset_095,genetic_effect_095 + clone_effect_095,gen_time,population)+min_age,NA)) %>%
    mutate(true_clone_age_intervals = 10*round(true_clone_age/10) + 5) %>%
    subset(Individual %in% gof_plot_values$Individual[gof_plot_values$sum_stat < 0.7])
})
...
x_offset_pointrange <- 5
y_offset_pointrange <- -10
legend_elements <- list(
  pointrange = data.frame(
    x = 45 + x_offset_pointrange,
    ymin = -40 + y_offset_pointrange,
    ymax = 0 + y_offset_pointrange,
    y = -20  + y_offset_pointrange
  ),
  text = data.frame(
    x = c(45,45,45) + 2 + x_offset_pointrange,
    y = c(-40,-20,0) + y_offset_pointrange,
    label = c("5% quantile","Mean","95% quantile")
  ),
  rect = data.frame(
    xmin = 42 + x_offset_pointrange,
    xmax = 80 + x_offset_pointrange,
    ymin = 10 + y_offset_pointrange,
...
x_offset_boxplot <- 0
y_offset_boxplot <- 0
legend_elements_boxplot <- list(
  boxplot = data.frame(
    x = 5 + x_offset_boxplot,
    y = c(105,110,125,140,145)  + y_offset_boxplot
  ),
  text = data.frame(
    x = 5 + 4 + x_offset_boxplot,
    y = 125 + y_offset_boxplot,
    label = "Distribution\nof means"
  ),
  rect = data.frame(
    xmin = 0 + x_offset_boxplot,
    xmax = 37.5+ x_offset_boxplot,
    ymin = 100 + y_offset_boxplot,
    ymax = 150 + y_offset_boxplot
  )
)

plot_min_max <- age_data %>% 
  subset(estimated_clone_age < 125 & estimated_clone_age > 0) %>% 
  subset(abs(estimated_clone_age_005) < 200)
plot_min_max <- c(
  min(plot_min_max$estimated_clone_age,
      plot_min_max$estimated_clone_age_095,
      plot_min_max$estimated_clone_age_005,
      plot_min_max$true_clone_age),
  max(plot_min_max$estimated_clone_age,
      plot_min_max$estimated_clone_age_095,
      plot_min_max$estimated_clone_age_005,
      plot_min_max$true_clone_age)
)
...
estimated_ages_plot <- age_data %>% 
  ...
  geom_rect(
    xmin = legend_elements$rect$xmin,
    xmax = legend_elements$rect$xmax,
    ymin = legend_elements$rect$ymin,
    ymax = legend_elements$rect$ymax,
    inherit.aes = F,
    fill = "white",
    colour = NA
  ) +
  geom_pointrange(inherit.aes = F,
                  size = 0.01,
                  colour = "grey",
                  y = legend_elements$pointrange$y,
                  x = legend_elements$pointrange$x,
                  ymax = legend_elements$pointrange$ymax,
                  ymin = legend_elements$pointrange$ymin) +
  geom_text(
  ...
  # begin legend for boxplot
  geom_rect(
    xmin = legend_elements_boxplot$rect$xmin,
    xmax = legend_elements_boxplot$rect$xmax,
    ymin = legend_elements_boxplot$rect$ymin,
    ymax = legend_elements_boxplot$rect$ymax,
    inherit.aes = F,
    fill = "white",
    colour = NA
  ) +
  geom_boxplot(inherit.aes = F,
               size = 0.2,width = 5,
               data = legend_elements_boxplot$boxplot,
               aes(x = x,
                   y = y)) +
  geom_text(
    data = legend_elements_boxplot$text,
    hjust = 0,
    inherit.aes = F,
    aes(x = x,y = y, label = label)
  ) + 
  # end legend for boxplot
...
estimated_ages_plot + ggsave(
  "tmp.pdf",height = 3.5,width = 3.5
)

age_data_full %>% 
  subset(estimated_clone_age < 125 & estimated_clone_age > -20) %>% 
  subset(abs(estimated_clone_age_005) < 200) %>% 
  ggplot(aes(x = true_clone_age,y = estimated_clone_age)) + 
  ...
  theme(legend.position = 'right') +
  scale_shape_discrete(name = "Generation time, population size") + 
  facet_grid(gen_time ~ population) 
```
...
cor_test_ages <- cor.test(age_data_post$true_clone_age,
                          age_data_post$estimated_clone_age)


cat(paste("Mean absolute error =",mean(abs(age_data_post$estimated_clone_age - age_data_post$true_clone_age)),'\n'))
cat(paste("Rsquared =",cor_test_ages$estimate^2,'\n'))
cat(paste('p-value =',cor_test_ages$p.value))
```

```{r age_residuals}
age_data_post$residuals_005 <- age_data_post$true_clone_age - age_data_post$estimated_clone_age_005
age_data_post$residuals <- age_data_post$true_clone_age - age_data_post$estimated_clone_age
age_data_post$residuals_095 <- age_data_post$true_clone_age - age_data_post$estimated_clone_age_095
...
car::qqPlot(age_data_post$residuals,
            col = colorRampPalette(c('orange','brown'))(5)[as.numeric(cut(age_data_post$fitness,breaks = 5))],
...
Next, we verify how much changing population sizes and time per generation affects estimates. To assess this we have calculated the values for different population sizes - 50,000 and 200,000 HSC - and for different generation times - 2 and 13 generations per year. For this, we observe that the median difference between all 4 possible combinations and after filtering according to the exclusion criteria cited above, is $\sim26$ for estimates for the upper bound, $\sim16$ for the median value, and $\sim11$ for the lower bound.

```{r compare_population_sizes_gen_time,fig.width=12,fig.height=6}
age_data_full %>%
  subset(estimated_clone_age < 125 & estimated_clone_age > -20) %>%
  subset(abs(estimated_clone_age_005) < 200) %>%
  group_by(individual_offset,true_clone_age) %>%
  summarise(m_005 = min(estimated_clone_age_005),
            M_005 = max(estimated_clone_age_005),
            m = min(estimated_clone_age),
            M = max(estimated_clone_age),
            m_095 = min(estimated_clone_age_095),
            M_095 = max(estimated_clone_age_095)) %>%
  na.omit() %>%
  mutate(Range_005 = M_005 - m_005,
         Range = M - m,
         Range_095 = M_095 - m_095) %>%
  ungroup() %>%
  summarise(Range_005Median = median(Range_005),
            RangeMedian = median(Range),
            Range_095Median = median(Range_095))
...
age_data_full %>%
  ...
  subset(estimated_clone_age < 125 & estimated_clone_age > -20) %>%
  ggplot(aes(x = true_clone_age,y = estimated_clone_age,
             ymin = estimated_clone_age_005,ymax = estimated_clone_age_095,
             color = paste(gen_time,population,sep='\n'))) + 
  geom_abline(intercept = 0,slope = 1,size = 2,alpha = 0.4,linetype=2) +
  geom_pointrange(alpha=0.5,position = position_dodge(width = 1.0)) +
  xlab("True clone age") +
  ylab("Estimated clone age") +
...
individuals_mutations <- age_data_post %>% 
  subset(estimated_clone_age_005 < 125 & estimated_clone_age_005 > -10) %>%
...
```{r investigating_sequencing_depths,fig.height=8,fig.width=5}
set.seed(4242)
random_samples <- list()
...
```{r sensitivity_of_sampling,fig.width=10}
all_simulated_samples <- list()
for (Coverage in c(20,50,100,1000,10000)) {
  all_simulated_samples[[Coverage]] <- lapply(seq(0,0.5,1e-4),function(Prob) {
    data.frame(
      coverage = Coverage,
      prob = Prob,
      samples = rbbinom(
...
all_simulated_samples_df <- all_simulated_samples %>%
  do.call(what = rbind)

all_simulated_Q <- all_simulated_samples_df %>% 
  group_by(coverage,prob) %>%
  summarise(q_005 = quantile(samples,0.05),
            q_025 = quantile(samples,0.25),
            q_050 = quantile(samples,0.5),
            q_075 = quantile(samples,0.75),
            q_095 = quantile(samples,0.95),
            mean = mean(samples)) 

all_simulated_Q %>%
  subset(prob <= 0.01) %>%
  subset(coverage >= 1000) %>%
  ggplot(aes(x = prob,
             y = q_050/coverage,
             group = as.factor(coverage),
             color = as.factor(coverage),
             fill = as.factor(coverage))) + 
  geom_ribbon(aes(ymax = q_095/coverage,ymin = q_005/coverage),
              alpha = 0.3) +
  geom_ribbon(aes(ymax = q_075/coverage,ymin = q_025/coverage),
              alpha = 0.4) +
  geom_point(alpha = 0.5) +
  geom_abline(slope = 1,intercept = 0) +
  geom_polygon(data = data.frame(x = c(0.005,0.005,0,0),
                                 y = c(0,0.005,0.005,0)),
               alpha=0.5,
               inherit.aes = F,
               aes(x = x,y = y)) +
  ...
  ggtitle("Distribution of simulated sequencing samples given their true proportion",
          subtitle = "Median (points), 50% CI (dark band) and 90% CI (lighter band) quantiles")
```
...
random_sample_df %>%
  ...
  ggplot(aes(x = NClonesAtThreshold,fill = as.factor(coverage))) + 
  geom_bar(color=NA) +
  facet_grid( ~ Gen) +
  theme_gerstung() +
  coord_flip() +
...
random_sample_df %>%  
  ...
  theme(legend.position = "bottom") +
  scale_x_continuous(breaks = seq(0,1300,130)) + 
  scale_y_continuous(name = "Detected clones/Detectable clones",
...
good_examples <- chosen_one_full %>% 
  subset((mutation_rate == "3.2e-09" & fitness == 0.011 & Replicate == 3) |
           (mutation_rate == "1.8e-09" & fitness == 0.010 & Replicate == 2) | 
           (mutation_rate == "2.95e-09" & fitness == 0.008 & Replicate == 2))
...
for (i in 1:nrow(good_examples[2,])) {
  try({
    ...
    driver_fit_coefficient <- lsfit(tmp$Gen / 13,logit(tmp$Count))$coefficients
    ...
    example_trajectory_plot <- example_trajectory_neutral %>% 
      ...
      annotate(
        ...
        alpha = 0.8) +
      # geom_point(
      #   data = age_at_clone_foundation_info,
      #   aes(x = x/13,y = y_points),
      #   shape=25,
      #   inherit.aes = F,
      #   size = 4,
      #   fill = 'black'
      ...
      # geom_text(
      #   data = age_at_clone_foundation_info,
      #   aes(x = x/13,y = y_points,label = label),
      #   inherit.aes = F,
      #   size = 4,
      #   angle = 90,
      #   hjust = -0.05,
...
main_results <- plot_grid(
  example_trajectory_plot + theme(legend.position = c(0.6,0.8)),
  plot_grid(
    predicted_coefficients_plot,
    estimated_ages_plot,
    labels = c("B","C"),
...
main_results
ggsave(main_results,filename = "figures/simulations/simulation_plot.pdf",height = 7,width = 10.5,
       useDingbats=FALSE)
