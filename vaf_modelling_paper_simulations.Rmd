...
y_offset_pointrange <- -5
legend_elements <- list(
  pointrange = data.frame(
    x = 45 + x_offset_pointrange,
    ymin = 5 + y_offset_pointrange,
    ymax = 15 + y_offset_pointrange,
    y = 10  + y_offset_pointrange
  ),
  text = data.frame(
    x = c(45,45,45) + 2 + x_offset_pointrange,
    y = c(5,10,15) + y_offset_pointrange,
    label = c("5% quantile","Mean","95% quantile")
  ),
  rect = data.frame(
    xmin = 42 + x_offset_pointrange,
    xmax = 80 + x_offset_pointrange,
    ymin = 3 + y_offset_pointrange,
    ymax = 17 + y_offset_pointrange
  )
)
...
legend_elements_boxplot <- list(
  boxplot = data.frame(
    ...
    y = c(100,102,104,106,108)  + y_offset_boxplot
  ),
  text = data.frame(
    x = 5 + 4 + x_offset_boxplot,
    y = 104 + y_offset_boxplot,
    label = "Distribution\nof means"
  ),
  rect = data.frame(
    xmin = 0 + x_offset_boxplot,
    xmax = 37.5+ x_offset_boxplot,
    ymin = 99 + y_offset_boxplot,
    ymax = 109 + y_offset_boxplot
  )
)
...
age_data_full %>% 
  ggplot(aes(x = true_clone_age,y = Mean)) + 
...
age_data_full %>% 
  group_by(gen_time,population) %>% 
  summarise(MAE = mean(abs(true_clone_age-Mean)))
```
...
cat(paste("Mean absolute error =",mean(abs(age_data$Mean - age_data$true_clone_age)),'\n'))
...
age_data_post$residuals <- age_data_post$true_clone_age - age_data_post$Mean
...
```{r compare_population_sizes_gen_time,fig.width=8,fig.height=6}
age_data_full %>%
...
residual_plot <- age_data_full %>%
  ...
  ggplot(aes(x = reorder(Individual,true_clone_age),y = true_clone_age - Mean,
...
age_data_full %>%
  ...
  theme_gerstung(base_size = 15) +
  theme(axis.text.x = element_blank()) + 
  xlab("Inferred age at onset") + 
  scale_colour_lancet(name = "Generations per year\nPopulation size") +
  ylab("Simulated age at onset") + 
  theme(legend.position = "bottom")
...
file_ids <- file_ids %>% 
  ...
  select(-div)
file_ids$mutation <- sapply(individuals_mutations$Individual,function(x) str_split(x,'_')[[1]][1]) %>%
  as.numeric
