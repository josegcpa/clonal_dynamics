...
model_a_metrics %>%
  ggplot() +
  geom_hline(aes(x = site,yintercept = average_r ** 2,colour = genes),stat = 'identity',
              alpha = 0.4) +
...
values_b <- values_model_a$b_values[[2]][grepl("b\\[",values_model_a$b_values[[1]]$variable),]
...
```{r model_A_clone_sizes, fig.width=10,fig.height=10}
full_data %>%
  subset(relative_timepoint == 1) %>%
  merge(values_b %>% subset(labels == 'mean')) %>%
  mutate(clone_age = log(1/VAF/2e5) / values) %>%
  mutate(clone_age = ifelse(is.infinite(clone_age),NA,clone_age)) %>%
  select(SardID,Age,amino_acid_change,Gene,Domain,clone_age) 
```
...
$$
Site\ effects: \mu_{site} \sim N(0,1),\sigma_{site} \sim logN(0,0.1)
\\
Domain\ effects: \mu_{domain} \sim N(0,1),\sigma_{domain} \sim logN(0,0.01)
\\
...
``` {r, model_c_validation, fig.width = 20,fig.height = 20}
val_subset <- formatted_data_validation_1
...
model_c_metrics %>%
  ...
  ggplot() +
  geom_hline(aes(x = site,color = genes,yintercept = average_r ** 2)) + 
...
```{r model_C_clone_sizes, fig.width=10,fig.height=10}
gps <- values_b_gene %>%
  subset(labels == 'mean') %>% 
  select(values) %>% t %*% t(formatted_data_train_1$gene_to_site_indicator)
dps <- values_b_domain %>%
  subset(labels == 'mean') %>% 
  select(values) %>% t %*% t(formatted_data_train_1$domain_to_site_indicator)
sps <- values_b_site %>%
  subset(labels == 'mean') %>% 
  select(values) %>% t %*% t(formatted_data_train_1$site_multiple_to_site_indicator)

full_effects <- data.frame(
  effects = c(gps + dps + sps),
  site = formatted_data_train_1$unique_site
)

full_data %>%
  subset(relative_timepoint == 1) %>%
  merge(full_effects,by.x = "amino_acid_change",by.y = "site") %>%
  mutate(clone_age = - log(1/VAF/2e5) / effects) %>%
  mutate(clone_age = ifelse(is.infinite(clone_age),NA,clone_age)) %>%
  select(SardID,Age,VAF,clone_age,Gene,Domain,effects,TOTALcount,amino_acid_change) %>% 
  mutate(growth_5yr = rbinom(1,gtools::inv.logit(effects * (Age + 5)),size = TOTALcount) - rbinom(1,gtools::inv.logit(effects * Age),n = TOTALcount))
```
