...
```{r recurrence_prevalence,warning=F,message=F,fig.height=4,fig.width=5.5}
...
Dens <- totalPlotData %>%
  ...
  geom_point(aes(size = Total),shape = "square",colour = "grey20") + 
  theme_gerstung(base_size = 15) + 
  geom_text(aes(x = "Total",y = N, label = Total),colour = 'black',vjust=-0.6,fontface="bold") + 
  theme_gerstung(base_size = 15) + 
  scale_size(range = c(1.5,10),limits = c(1,max(totalPlotData$Total)),guide = F) + 
  xlab("") + 
  ylab("") + 
  theme(panel.border = element_blank())

PoissonTotalLambda <- sum(totalPlotData$N * totalPlotData$Total) / length(unique(load_data_keep()$SardID))
totalPlotData$Significant <- ifelse(
  dpois(totalPlotData$N,lambda = PoissonTotalLambda) < (0.05 / length(totalPlotData$N)),
  "Significant",
  "Non-significant"
)

NMutGenesPlot <- PlotData %>% 
  ggplot(aes(x = Gene,y = N, shape = Significant)) +
  geom_point(aes(size=Total,color = Significant)) + 
  geom_point(
    data = totalPlotData,
    aes(x = Gene,y = N,size = Total,colour = Significant,shape = Significant),
    inherit.aes = F
  ) + 
  geom_label(
    data = totalPlotData,
    aes(x = Gene,y = N,label = Total),
    alpha = 0.8,
    inherit.aes = F,
    vjust = -0.6,
    colour = "black",
  ...
  #            ) + 
  geom_label(aes(x = Gene,y = N,label = Total),
             alpha = 0.8,
             vjust=-0.6,
             colour = "black",
             label.padding = unit(0,"cm"),
  ...
  #                       name = 'log(p)') + 
  scale_colour_manual(breaks = c("Significant","Non-significant"),
                      values = c("black","#d3d3d3"),
                      name = NULL) +
  scale_shape(breaks = c("Significant","Non-significant"),
              name = NULL) +
  rotate_x_text() + 
  ylab("Number of unique mutations\nper individual") + 
  scale_size(range = c(1.5,9),limits = c(1,max(totalPlotData$Total)),guide = F) +
  scale_y_continuous(limits = c(1,NA),expand = c(0.07,0,0.1,0),
                     breaks = seq(1,10)) + 
  theme(legend.position = c(0.2,0.9),
        legend.margin = margin(0,0.1,0.1,0.1,"cm"),
        legend.background = element_rect(fill = NA,colour = "black")) +
  scale_x_discrete(
    labels = c(sapply(unique(PlotData$Gene),function(x) parse(text = sprintf('italic("%s")',x))),
               Total = parse(text = "bold(Total)")),
    limits = c(unique(PlotData$Gene),"Total")
  )
NMutGenesPlot + 
  ggsave("figures/data_exploration/PrevalenceByGene.pdf",height = 4,width = 5.5)
```

```{r prevalence_through_time,fig.height=3,fig.width=5}
full_data %>% 
  group_by(SardID,Phase,Age) %>%
  summarise(VAF = mean(VAF)) %>%
  mutate(AgeGroup = Age %/% 10 * 10) %>% 
  subset(AgeGroup < 100) %>%
  ...
  scale_x_continuous(breaks = seq(50,100,by = 10),
                     labels = paste(seq(50,100,by = 10),seq(60,110,by=10),sep = '-')) + 
  ylab("Average variant\nallele frequency") + 
  ggsave(useDingbats=FALSE,"figures/data_exploration/mean_vaf_through_time.pdf",width=5, height=3)
...
plot_grid(age_per_phase_plot,n_mutations_age,mutations_per_gene_time,
  ...
  ggsave(useDingbats=FALSE,"figures/data_exploration/summary_figure.pdf",width=10, height=5)
...
\\
p_i = ilogit((b_{gene_i} + b_{site_i} + b_{clone_{ji}}) * age + u_{ji})
\\
...
r_values_full <- data.frame(
  ...
  b_gene_005 = c(values_model$b_gene_values[[1]]$values[values_model$b_gene_values[[1]]$labels == 'HDPI_low'][values_model$sub_data$gene_numeric]),
  b_gene = c(values_model$b_gene_values[[1]]$values[values_model$b_gene_values[[1]]$labels == 'mean'][values_model$sub_data$gene_numeric]),
  b_gene_095 = c(values_model$b_gene_values[[1]]$values[values_model$b_gene_values[[1]]$labels == 'HDPI_high'][values_model$sub_data$gene_numeric]),
  b_gene_var = c(values_model$b_gene_values[[1]]$values[values_model$b_gene_values[[1]]$labels == 'var'][values_model$sub_data$gene_numeric]),
  b_site_005 = c(values_model$b_site_values[[1]]$values[values_model$b_site_values[[1]]$labels == 'HDPI_low'][values_model$sub_data$site_numeric_multiple]),
  b_site = c(values_model$b_site_values[[1]]$values[values_model$b_site_values[[1]]$labels == 'mean'][values_model$sub_data$site_numeric_multiple]),
  b_site_095 = c(values_model$b_site_values[[1]]$values[values_model$b_site_values[[1]]$labels == 'HDPI_high'][values_model$sub_data$site_numeric_multiple]),
  b_site_var = c(values_model$b_site_values[[1]]$values[values_model$b_site_values[[1]]$labels == 'var'][values_model$sub_data$site_numeric_multiple]),
  b_clone_005 = c(values_model$b_clone[[1]]$values[values_model$b_clone[[1]]$labels == 'HDPI_low'][values_model$sub_data$clone]),
  b_clone = c(values_model$b_clone[[1]]$values[values_model$b_clone[[1]]$labels == 'mean'][values_model$sub_data$clone]),
  b_clone_095 = c(values_model$b_clone[[1]]$values[values_model$b_clone[[1]]$labels == 'HDPI_high'][values_model$sub_data$clone]),
  b_clone_var = c(values_model$b_clone[[1]]$values[values_model$b_clone[[1]]$labels == 'var'][values_model$sub_data$clone]),
  u_values_005 = c(values_model$u_values[[1]]$values[values_model$u_values[[1]]$labels == 'HDPI_low'][values_model$sub_data$clone]),
  u_values = c(values_model$u_values[[1]]$values[values_model$u_values[[1]]$labels == 'mean'][values_model$sub_data$clone]),
  u_values_095 = c(values_model$u_values[[1]]$values[values_model$u_values[[1]]$labels == 'HDPI_high'][values_model$sub_data$clone])
) %>%
  mutate(
  ...
  apply(1,FUN = function(x){
    ...
    mu_val <- inv.logit(unlist((b_gene + b_site + b_clone) * (age - values_model$min_age) + u)) / 2
...
```{r model_dynamic_visualization,warning=F,fig.width = 7.8,fig.height = 5}
...
r_values %>% 
  ...
  distinct %>% 
  subset(site %in% values_model$sub_data$amino_acid_change[values_model$sub_data$single_occurring == F]) %>% 
...
```{r model_f2_full_effects, fig.width=10, fig.height=4,warning=F}
...
```{r model_f2_full_effects_simplified, fig.width=7.7, fig.height=4,warning=F}
...
plot_growth_per_year <- gene_intervals %>% 
  ...
  geom_linerange(
    ...
    size = 0.1) +
  geom_linerange(
    data = unknown_cause_growth,
    inherit.aes = F,
    aes(ymin = exp(b_clone_005)*100-100,
        ymax = exp(b_clone_095)*100-100,
        group = reorder(paste(site,individual),b_clone),
        colour = gene,
        alpha = 0,
        x = "uc"),
    size = 0.1,
  ...
  scale_x_discrete(breaks = c("FALSE","TRUE","uc"),
                   labels = c("Non-trunc.","Trunc.","Unk. cause"),
                   expand = c(0.01,0.01)) +
  scale_alpha(range = c(0.3,1.0),guide = F) +
  guides(size = guide_legend(nrow = 1,direction = "horizontal")) 
...
for (i in axis_x) {
  g$grobs[[i]]$children$axis$grobs$`1`$children[[1]]$gp$font <- ifelse(
    g$grobs[[i]]$children$axis$grobs$`1`$children[[1]]$label == 'Unk. cause',
    3,
    1
  ) %>%
    as.integer()
...
ggsave(useDingbats=FALSE,plot = gridExtra::grid.arrange(g),
       sprintf("figures/%s/dynamic_coefficients/FigureFullSimplified.pdf",model_id),
       width = 7.7,height = 3.3) 
ggsave(plot = gridExtra::grid.arrange(g),
       sprintf("figures/%s/dynamic_coefficients/FigureFullSimplified.svg",model_id),
       width = 7.7,height = 3.3)
```
...
r_values %>%
  subset(site %in% unique_site_multiple) %>% 
  group_by(individual,site,b_clone,genes) %>%
  summarise(age = min(age)) %>% 
  ungroup %>% 
  mutate(genes = str_match(genes,'[A-Z0-9]+')) %>% 
...
var.test(mutation_independent_coefficients$b_clone[mutation_independent_coefficients$genes == 'TET2'],
         mutation_independent_coefficients$b_clone[mutation_independent_coefficients$genes == 'DNMT3A'],
         alternative = "greater")
```
...
```{r compare_gene_unknown_cause,fig.height=3.3,fig.width=3.3}
good_stat_sites <- statistic_full %>%
  subset(sum_stat < 0.7) %>% 
  select(site) %>% 
  unlist
Parameters <- r_values_full %>% 
  select(b_clone_mean = b_clone,b_clone_var,b_clone_005,b_clone_095,
         ...
         site) %>%
  mutate(
    b_genetic_mean = b_gene_mean + b_site_mean,
    b_genetic_var = b_gene_var + b_site_var,
    b_genetic_005 = b_gene_005 + b_site_005,
    b_genetic_095 = b_gene_095 + b_site_095
  ) %>% 
...
ggplot(Parameters,aes(x = exp(b_clone_mean) * 100 - 100,
                      xmin = exp(b_clone_005) * 100 - 100,
                      xmax = exp(b_clone_095) * 100 - 100,
  ...
  scale_colour_manual(values = c(gene_colours,gene_colours_lighter),
                      guide = F) + 
  ggsave(useDingbats=FALSE,sprintf("figures/%s/dynamic_coefficients/UnknownVsDriver.pdf",model_id),
...
clone_ages <- r_values %>%
  ...
  mutate(CloneAgeHenryLower_adj = t0_adjusted(u_values,coefficient + b_clone,13,5e4 * 2) + values_model$min_age,
         CloneAgeHenryUpper_adj = t0_adjusted(u_values,coefficient + b_clone,13,2e5 * 2) + values_model$min_age
         )
...
data_plot_clone_ages_summary <- data_plot_clone_ages %>%
  subset(aa_org %in% unique_site_multiple) %>%
...
```{r age_at_clone_foundation_intervals,fig.height=4.7,fig.width=6.3,warning=F}
...
sample_ages <- function(sampling_idxs,pop_size,gen_time) {
  sampling_idxs %>% 
    apply(1,function(x) {
      ...
      if (!is.na(site_numeric)) {
        b_site <- sample(site_samples[,site_numeric],1000)
      } else {
        b_site <- 0
      }
      b_gene <- sample(gene_samples[,gene_numeric],1000)
      b_clone <- sample(clone_samples[,clone_numeric],1000)
      u <- sample(offset_samples[,clone_numeric],1000)
      ...
      colnames(tmp) <- c("age_distribution","effect_distribution","site","individual","gene","t1")
      return(tmp)
      }) %>%
    return
}
...
for (gen_time in c(1,2,3,5,6,10,13)) {
  for (pop_size in c(5e4,1e5,1.5e5,2e5)) {
    tmp_samples <- sample_ages(sampling_idxs,pop_size,gen_time)
...
all_samples_for_age_estimation <- list_of_age_sampling$`1e+05_13`
age_estimates <- list_of_age_estimates$`1e+05_13`
...
lapply(names(list_of_age_estimates),
       function(x) {
           ...
           colnames(o) <- c("MAP","Count","pop_size","gen_time")
           return(o)
       }) %>% 
  do.call(what = rbind) %>% 
  as_tibble() %>% 
  group_by(gen_time,pop_size) %>% 
  summarise(Freq = Count[MAP == -1]/sum(Count)) %>% 
  ...
  scale_fill_material(reverse = F,
                      name = "No. of developmental clones") + 
  scale_x_discrete(expand = c(0,0)) + 
  scale_y_discrete(expand = c(0,0)) + 
  theme(legend.position = "bottom") + 
  ggsave(useDingbats=FALSE,sprintf("figures/%s/age_at_clone_foundation/MAP.pdf",model_id),
         height = 6,width = 6)
...
Parameters_Age <- cbind(
  ...
  merge(Parameters,by = c("individual","site")) %>%
  mutate(
...
sample_to_display <- sampling_idxs %>%
  subset(site %in% M$site & individual %in% M$individual & clone_numeric %in% M$clone_numeric) %>% 
  apply(1,function(x) {
    site_numeric <- as.numeric(x[3])
    gene_numeric <- as.numeric(x[4])
    clone_numeric <- as.numeric(x[5])
    ...
    if (!is.na(site_numeric)) {
      b_site <- sample(site_samples[,site_numeric],500)
    } else {
      b_site <- 0
    }
    b_gene <- sample(gene_samples[,gene_numeric],500)
    b_clone <- sample(clone_samples[,clone_numeric],500)
    u <- sample(offset_samples[,clone_numeric],500)

    t1 <- as.numeric(x[6])
    age_distribution <- t0_adjusted(u,b_site+b_gene+b_clone,13,2e5 * 2) + values_model$min_age
...
kd <- ks::kde(sample_to_display_large[,c(1,2)], compute.cont=TRUE)
contour_95 <- with(kd, contourLines(x=eval.points[[1]], y=eval.points[[2]],
                                    z=estimate, levels=cont["5%"])[[1]])
contour_95_large <- data.frame(contour_95)


kd <- ks::kde(sample_to_display_other[,c(1,2)], compute.cont=TRUE)
contour_95 <- with(kd, contourLines(x=eval.points[[1]], y=eval.points[[2]],
                                    z=estimate, levels=cont["5%"])[[1]])
...
age_at_detection <- 90
ages <- seq(0,100,length.out = 400)
fitness <- exp(seq(0.0001,max(Parameters_Age$b_genetic_mean + Parameters_Age$b_clone_mean)+0.1,length.out = 1000)) - 1
detection_limit <- sapply(fitness, function(f) sapply(ages, function(t) trajectory(age_at_detection, t, f, N,13)/N))
detection_limits <- data.frame(
  t0 = apply(detection_limit,2,
             function(x) {
               A <- suppressWarnings(approx(x/2,ages,xout = detection_threshold)$y)
               if (length(A) > 0) {
                 return(A)
               } else {
                 return(0)
...
detection_limits <- rbind(
  detection_limits,
  data.frame(t0 = c(max(detection_limits$t0),0),fitness = c(max(fitness),0))
)
...
Parameters_Age %>%
  ...
  geom_polygon(data = detection_limits,
               ...
               fill = "grey95",
               aes(x = t0,y = fitness)) +
  geom_linerange(size = 0.5,aes(colour = gene_lighter)) + 
  geom_errorbarh(size = 0.5,aes(colour = gene_lighter)) +
  geom_point(size = 1.5) + 
  # geom_polygon(
...
geom_line(data = detection_limits[1:(nrow(detection_limits)-3),],
          aes(x = t0,y = fitness),
          inherit.aes = F) + 
annotate(geom = 'text',
         x = detection_limits$t0[which.max(detection_limits$fitness)],
         y = 0,
         label = "Detection\nthreshold",
         vjust = -0.1,
         hjust = 1,
  ...
  ylab("Total growth per year") +
  xlab("Age at onset") +
  theme_gerstung(base_size = 15) + 
  scale_color_manual(values = c(gene_colours,gene_colours_lighter),
                     guide = F) + 
  ggsave(useDingbats=FALSE,sprintf("figures/%s/age_at_clone_foundation/Growth_vs_Age_at_onset.pdf",model_id),width = 6.3,height = 4.7) +
  ggsave(sprintf("figures/%s/age_at_clone_foundation/Growth_vs_Age_at_onset.svg",model_id),width = 6.3,height = 4.7) 
...
Parameters_Age %>% 
  ...
  write.csv("ParametersAgeAtOnset.csv",quote = F,row.names = F)
```

```{r ages_density,fig.height=4.7,fig.width=4.7}
gene_order_age <- Parameters_Age %>%
  mutate(Gene = str_match(gene,'[0-9A-Z]+')) %>% 
  group_by(Gene) %>%
  summarise(m = median(Q50)) %>%
  ungroup() %>%
  arrange(m) %>% 
  select(Gene) %>%
  unlist
...
Parameters_Age %>%
  ...
  scale_shape_discrete(breaks = c(TRUE,FALSE),
                       labels = c("Truncating","Non-truncating"),
                       name = NULL) +
  scale_y_continuous(breaks = seq(0,100,25),expand = c(0.01,0.01),limits = c(0,90)) +
  ylab("Age at clone foundation") + 
  xlab("Driver gene") +
  coord_flip() + 
  ggsave(useDingbats=FALSE,sprintf("figures/%s/age_at_clone_foundation/Figure6_Boxplots.pdf",model_id),width = 4.7,height = 4.7) +
  ggsave(sprintf("figures/%s/age_at_clone_foundation/Figure6_Boxplots.svg",model_id),width = 4.7,height = 4.7) 
```

```{r ages_density_bar,fig.height=4.7,fig.width=4.7}
estimates_from_samples <- all_samples_for_age_estimation_gene %>%
  lapply(function(x) {
    X <- lapply(x,function(y) {
      ...
      as.vector
    gene <- x[[1]]$gene[1]
    output <- c(mean(X),quantile(X,c(0.05,0.50,0.95)))
    data.frame(
      labels = c("Mean","Q05","Q50","Q95"),
      gene = rep(gene,4),
      values = output,
      row.names = NULL
    ) %>% 
...
gene_order <- estimates_from_samples %>%
  arrange(Q95) %>%
  select(gene) %>%
  unlist

max_growth_fitness <- Parameters_Age %>%
  group_by(gene) %>%
  summarise(MaxGrowth = max(exp(b_gene_mean + b_clone_mean + b_site_mean)) - 1) %>%
  rowwise() %>%
  mutate(MaxAgeAtDetection = seq(-1,110,by = 0.1)[
    which(diff(trajectory(100,seq(-1,110,by = 0.1),MaxGrowth) > (2e5 * 0.001)) != 0)]) %>% 
  mutate(AgeAtSampling = 100)

estimates_from_samples %>% 
  ggplot(aes(ymin = Q05,ymax = Q95,
             x = factor(gene,levels = gene_order),
             colour = gene)) +
  geom_linerange(data = max_growth_fitness,
                 aes(x = factor(gene,levels = gene_order),
                     ymin = MaxAgeAtDetection,
                     ymax = AgeAtSampling),
                 colour = "grey",
                 size = 5) + 
  ...
  scale_colour_manual(values = gene_colours,
                      guide = F) + 
  ggsave(useDingbats=FALSE,sprintf("figures/%s/age_at_clone_foundation/AgeAtOnsetBars.pdf",model_id),width = 4.7,height = 4.7) +
  ggsave(sprintf("figures/%s/age_at_clone_foundation/AgeAtOnsetBars.svg",model_id),width = 4.7,height = 4.7) 
```
...
data_growth_age_association <- Parameters_Age %>%
  mutate(Truncating = grepl("[A-Z0-9]+t",gene),
         Gene = str_match(gene,"[A-Z0-9]+"),
         amino_acid_change = as.character(site)) %>%
  subset(amino_acid_change %in% unique_site_multiple) %>%
  mutate(amino_acid_change = sapply(amino_acid_change,function(x) unlist(strsplit(x,'-'))[2])) %>%
  mutate(amino_acid_change = sapply(amino_acid_change,function(x) unlist(strsplit(x,';'))[1])) %>%
  mutate(amino_acid_change = sapply(amino_acid_change,function(x) {
    tmp <- unlist(strsplit(x,':'))
  ...
  select(site,individual,
         coefficient = b_genetic_mean,
         coefficient_005 = b_genetic_005,
         coefficient_095 = b_genetic_095,
         b_clone_mean,b_clone_005,b_clone_095,Q05,Q50,Q95,Gene) %>% 
  distinct()

C <- cor.test(
  data_growth_age_association$coefficient,
  data_growth_age_association$Q50)
TEXT <- c(
...
wilcox.test(square_growth_age_test$Q50[square_growth_age_test$S == 'Slow'],
            square_growth_age_test$Q50[square_growth_age_test$S == 'Fast'],
            alternative = "less")
square_growth_age <- data_growth_age_association %>%
  mutate(S = ifelse(coefficient < 0.2,"Slow","Fast")) %>% 
  group_by(S) %>%
  summarise(min_age = min(Q50),
            max_age = max(Q50),
...
data_growth_age_association %>% 
  ggplot(aes(y = exp(coefficient)*100-100,
             x = Q50,
             xmin = Q05,
             xmax = Q95,
...
plot_grid(
  ...
  ggsave(useDingbats=FALSE,"figures/model_F5/statistical_analysis/performance_and_predicted_6thpoint_trajectories.pdf",height = 4,width = 7)
```
...
most_recurring_coefficients_gene <- r_values %>% 
...
## Comparing dynamics and age at onset - trees and longitudinal data

```{r loading_trees_bnpr_estimates}
trees <- load_trees()
...
```{r comparing_estimates,fig.width=13.3,fig.height=5.8}
clone_assignment <- list(
  `2259` = list(PD41276k_25 = "SF3B1-K666N",
                PD41276k_50 = "Unknown driver"),
...
max_n_clades <- max(unlist(lapply(bnpr_estimates, function(x) length(x$clade_dynamics))))
N_clades <- sum(unlist(lapply(bnpr_estimates, function(x) length(x$clade_dynamics))))
pdf(file = sprintf("figures/%s/trees/all_trees_and_dynamics.pdf",model_id),
    width = 7,
    height = 3 * N_clades
    )
layout(matrix(c(1:(N_clades*2)),N_clades,2,byrow = TRUE),
       widths = c(0.7,1))
for (curr_id in names(clone_assignment)) {
  O <- bnpr_estimates[[curr_id]]
  max_age <- ages[[curr_id]]
  for (clade in O$clade_dynamics) {
    linear_estimate <- clade$linear_estimate
    tree_ultra <- O$tree_ultra
    tree_ultra$edge.length <- tree_ultra$edge.length * max_age
    C <- seq(1:length(tree_ultra$tip.label))
    C[C %in% clade$clade[tree_ultra$edge[,2]]] <- "red3"
    C[C != "red3"] <- "black"
    e <- C[tree_ultra$edge[,2]]
    q <- tree_ultra$edge[is.na(e),2]
    ii <- 0
    while(length(q)>0 & ii<10000){
  		k <- q[1]
  		q <- q[-1]
  		children <- which(tree_ultra$edge[,1]==k)
  		#print(c(k, e[l]))
  		ee <- NA
  		if(sum(!is.na(e[children]))>0){
  			if(all(!is.na(e[children]))){
  				if(length(unique(e[children]))==1)
  					ee <- unique(e[children])
  			} else {
  				parent <- tree_ultra$edge[tree_ultra$edge[,2]==k,1]
  				sibling <- which(tree_ultra$edge[,1]==parent & tree_ultra$edge[,2]!=k)
  				if(!is.na(e[sibling]))
  					if(na.omit(e[children])==e[sibling])
  						ee <- e[sibling]
  			}
  		}
  		e[tree_ultra$edge[,2]==k] <- ee
  		if(is.na(ee)) q <- c(q,k)
  		ii <- ii + 1
	  }
    
    edge_widths <- ifelse(e == 'black',0.8,1.2)
    par(mar = c(4,1,1,1))
    plot(tree_ultra,
         tip.color = C,show.tip.label = F,edge.color = e,edge.width = edge_widths)
    axis(side = 1,cex.axis = 1)
    mtext("Age",side = 1,line = 2,cex = 1.0)
    ...
    if (driver_id != "Unknown Driver") {
      ...
      fitness <- 1 / b / 13
      pop_size <- 2e5
      drift_threshold <- 1 / (fitness * pop_size) / 2
      time_at_drift <- (logit(drift_threshold) - u)/m
      ...
      optim_func <- function(params) {
        ...
        return(mean((y * scaling_factor - bnpr_y)^2))
      }
      solution <- optim(par = c(scaling_factor = 1e3),optim_func,method = "Brent",
                        lower = 1, upper = 1e10)
  
      y <- ifelse(bnpr_x < age_at_onset[2],NA,y)
      y_05 <- ifelse(bnpr_x < age_at_onset[2],NA,y_05)
      y_95 <- ifelse(bnpr_x < age_at_onset[2],NA,y_95)
      
      y_lims <- c(
        min(y * solution$par,bnpr_y),
        max(y * solution$par,bnpr_y)
      )
      ...
      y_axis_at_vaf <- c(10^c(-4:-1),0.5)
      
      par(mar=c(4, 5, 4, 5))
      plot(bnpr_x,bnpr_y,log = "y",bty="n",type = "l",xlab = NA,ylab = NA,
           axes = F,ylim = y_lims)
      polygon(c(bnpr_x,rev(bnpr_x)),
              c(clade$bnpr_estimate$summary$quant0.975,rev(clade$bnpr_estimate$summary$quant0.025)),
              col = "pink", border = NA)
      axis(1, at = x_axis_at,
           labels = x_axis_at)
      axis(2,at = y_axis_at,las=1,
           labels = format(y_axis_at,scientific=F))
      mtext("Age",side = 1,line = 2,cex = 1.0)
      mtext("Effective pop. size (EPS)",side = 2,line = 4,cex = 0.8)
      # polygon(c(bnpr_x,rev(bnpr_x)),
      #         solution$par * c(y_95,rev(y_05)),
      #         col = "grey90", border = NA)
      lines(bnpr_x,bnpr_y,col = "red",lwd = 2)
      par(new=TRUE)
      plot(bnpr_x,y * solution$par,axes=F,log = "y",type = "l",
           ylab = NA,xlab = NA,ylim = y_lims,lwd=2)
      #abline(h = drift_threshold * solution$par)
      axis(4,at = c(y_axis_at_vaf) * solution$par,
           labels = c(y_axis_at_vaf),ylim = y_lims,
           las=1)
      points(x = x_data,y = y_data * solution$par,cex = 2)
      mtext("Variant allele frequency (VAF)",side=4,line = 4,cex = 0.8)
      legend("topleft",legend=c("EPS (mean)",NA,"Inferred VAF","Data"),
             bty = "n",
             text.col="black",#c("red",NA,"black","black"),
             col=c("red",NA,"black","black"),
             pch = c("-",NA,"-","O"))
      legend("topleft",legend=c(NA,"EPS (95% CI)",NA,NA),
             bty = "n",
             text.col="black",#c(NA,"pink",NA,NA),
             col=c(NA,"pink",NA,NA),
             pch = c(NA,15,NA,NA))
      legend("bottomright",
             legend=c(sprintf("Growth/year (longitudinal) = %.4f",b),
                      sprintf("Growth/year (colonies) = %.4f",linear_estimate$coefficients[2])),
             bty = "n",
             text.col=c("black","red"))
      title(main = sprintf("SardID: %s (%s)",curr_id,driver_id))
      ...
      polygon(c(bnpr_x,rev(bnpr_x)),
              ...
              col = "pink", border = NA)
      lines(bnpr_x,bnpr_y,col = "red",lwd = 2)
      legend("topleft",legend=c("EPS (mean)",NA),
             bty = "n",
             text.col="black",#c("red",NA),
             col=c("red",NA),
             pch = c("-",NA))
      legend("topleft",legend=c(NA,"EPS (95% CI)"),
             bty = "n",
             text.col="black",#c(NA,"pink"),
             col=c(NA,"pink"),
             pch = c(NA,15))

      title(main = sprintf("SardID: %s (%s)",curr_id,driver_id))
      legend("bottomright",
             legend=c(sprintf("Growth/year (colonies) = %.4f",linear_estimate$coefficients[2])),
             bty = "n",
             text.col=c("red"))
  }
  }
...
plot_list <- list()
for (curr_id in c("3877","2259","500")) {
  O <- bnpr_estimates[[curr_id]]
  max_age <- ages[[curr_id]]
  if (curr_id == "2259") {
    clade_dynamics <- list(O$clade_dynamics[[1]])
  } else if (curr_id == "500") {
    clade_dynamics <- list(O$clade_dynamics[[1]])
  } else {
    clade_dynamics <- O$clade_dynamics[c(4,3)]
  }
  for (clade in clade_dynamics) {
    ...
    tree_groups <- groupOTU(tree_ultra,clade$clade) 
    tree_plot <- ggtree(tree_groups,aes(colour=group),size = 0.3,
                        ladderize=T) + 
      scale_colour_manual(values = c(`0` = "black",`1` = "red3"),
                          guide=F) + 
      theme_gerstung(base_size = 15) +
    ...
    if (driver_id != "Unknown Driver") {
      ...
      y_lims <- c(
        max(1,min(c(y * solution$par,bnpr_y,tmp_data$VAF))),
        max(c(y * solution$par,bnpr_y,tmp_data$VAF))
      )
      ...
      trajectory_plot <- data.frame(y = y * solution$par,
        ...
        geom_ribbon(aes(x = bnpr_x,ymin = bnpr_y025,ymax = bnpr_y975,
                        fill = "EPS (mean and 95% CI)",
                        shape = "EPS (mean and 95% CI)")) +
        geom_line(aes(x = bnpr_x,y = bnpr_y,colour = "EPS (mean and 95% CI)",
                      shape = "EPS (mean and 95% CI)"),
                  size = 1) + 
        geom_line(aes(x = bnpr_x,y = y,fill = "VAF (inferred)",colour = "VAF (inferred)",
                      shape = "VAF (inferred)"),
                  size = 1) + 
        geom_point(inherit.aes = F,
                   ...
                   aes(x = Age,
                       ...
                       colour = "Data")) +
        annotate("text",
                 label = c(sprintf("Growth/year (longitudinal) = %.4f\n",b),
                           sprintf("\nGrowth/year (colonies) = %.4f",linear_estimate$coefficients[2])),
                 x = max_age,
                 y = y_lims[1],
                 colour = c("black","red3"), 
                 hjust = 1,
        ...
        scale_y_continuous(
          ...
          breaks = 10^seq(1,10),
          labels = sprintf("10e%s",seq(1,10))) + 
        scale_x_continuous(expand = c(0.02,0,0.02,0)) +
        ylab("Effective  pop. size (EPS)") + 
        xlab("Age") + 
        coord_cartesian(ylim = y_lims) + 
        scale_colour_manual(values = c(`EPS (mean and 95% CI)` = "red3",
                                       `VAF (inferred)` = "black",
                                       `Data` = "black"),name = NULL,
                            breaks = c("EPS (mean and 95% CI)","VAF (inferred)","Data")) + 
        scale_fill_manual(values = c(`EPS (mean and 95% CI)` = "#ffd9df",
                                     `VAF (inferred)` = NA,
                                     `Data` = NA),name = NULL,
                          breaks = c("EPS (mean and 95% CI)","VAF (inferred)","Data")) + 
        scale_shape_manual(values = c(`EPS (mean and 95% CI)` = NA,
                                      `VAF (inferred)` = NA,
                                      `Data` = 1),name = NULL,
                           breaks = c("EPS (mean and 95% CI)","VAF (inferred)","Data")) +
        theme(legend.position = c(0.4,1.11),
      ...
      trajectory_plot <- data.frame(bnpr_x = bnpr_x,bnpr_y = bnpr_y,
        ...
        xlab("Age") + 
        coord_cartesian(ylim = y_lims) + 
        scale_colour_manual(values = c(`EPS (mean and 95% CI)` = "red3"),name = NULL) + 
        scale_fill_manual(values = c(`EPS (mean and 95% CI)` = "#ffd9df"),name = NULL) + 
        scale_shape_manual(values = c(`EPS (mean and 95% CI)` = NA),name = NULL) + 
        theme(legend.position = c(0.4,1.11),
              legend.direction = "vertical",
              legend.justification = c(0.5,1),
              legend.text.align = 0) 
      }
    plot_list[[sprintf('%s_%s_tree',curr_id,driver_id)]] <- tree_plot
    plot_list[[sprintf('%s_%s_trajectory',curr_id,driver_id)]] <- trajectory_plot
  }
}

plot_grid(
  plot_grid(plotlist = lapply(plot_list[c(1,3)],
                              function(x) x + theme(plot.margin = unit(c(0.4,0,0.1,0.5), "cm"))),
            align = "v",
            ncol = 1),
  plot_grid(plotlist = lapply(plot_list[c(2,4)],
                              function(x) x + theme(plot.margin = unit(c(0.4,0,0.1,0.5), "cm"))),
            align = "v",
            ncol = 1),
  plot_grid(plotlist = lapply(plot_list[c(5,7)],
                              function(x) x + theme(plot.margin = unit(c(0.4,0,0.1,0.5), "cm"))),
            align = "v",
            ncol = 1),
  plot_grid(plotlist = lapply(plot_list[c(6,8)],
                              function(x) x + theme(plot.margin = unit(c(0.4,0,0.1,0.5), "cm"))),
            align = "v",
            ncol = 1),
  ncol = 4,
  rel_widths = c(0.4,1,0.4,1)
) + 
  ggsave(filename = sprintf("figures/%s/trees/all_trees_and_dynamics_subset_ggplot.pdf",model_id,curr_id),
         width = 13.3,
         height = 5.8,
         useDingbats = F)
```
...
statistics_data_bars <- statistics_data %>%
  ungroup() %>%
  mutate(model_label = sprintf("Trajectories compatible\nwith model = %s%%",100*round(sum(sum_stat < 0.7)/length(sum_stat),3))) %>% 
  mutate(good = sum_stat < 0.7,
...
save(r_values,statistics_data,Parameters_Age,file = sprintf('vaf_modelling_coefficients_%s.Rdata',model_id))
```
